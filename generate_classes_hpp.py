...
def generate_member_getter(class_,member):
    ...
    res += f"{INDENT}"
    res = generate_newline(res)
    res += f"{INDENT}{member.type_} get_{member.name}();"
    res = generate_newline(res)
    return res

def generate_member_setter(class_,member):
    res = ""
    res += f"{INDENT}void set_{member.name}({member.type_} value);"
    return res
...
def generate_property(property):
    result = ""
    result += f"{INDENT}{unbitfield_type(unenumize_type((property['type'])))} get_{pythonize_name(property['name'])}()"+";"
    result = generate_newline(result)

    if "setter" in property and property["setter"] != "":
        result += f"{INDENT}void set_{pythonize_name(property['name'])}(  {untypearray(simplify_type(property['type']))} value);"
        result = generate_newline(result)
...
def generate_args(method_with_args):
    ...
    for arg in method_with_args["arguments"]:
        if not arg["type"].startswith("enum::"):
            result += f"{pythonize_name(arg['name'])}:{untypearray(unbitfield_type(arg['type']))}{generate_default_arg(arg,arg['type'])}, "
        else:
            #enums are marked with enum:: . To be able to use this, we have to strip this
            arg_type = arg["type"].replace("enum::", "")
            result += f"{pythonize_name(arg['name'])}:{untypearray(unenumize_type(arg_type))} {generate_default_arg(arg,arg['type'])}, "
    result = result[:-2]
    return result
...
def generate_copy_methods(class_name):
    res = ""
    res += f"{INDENT}def copy_from_other( {class_name} from_):"
    res = generate_newline(res)
