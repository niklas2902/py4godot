...
void create_signal_arg(const char* name, int variant_type, Array& args_dicts){
    ...
    Variant var_name_arg =
Variant::new_static(functions::get_dictionary_operator_index()(&arg_dict.godot_owner,
&arg_name_key_variant.native_ptr)); auto name_converted =
c_string_to_string(name); var_name_arg.init_type(name_converted);

    String arg_type_key = c_string_to_string("type");
    Variant arg_type_key_variant = Variant(arg_type_key);

    Variant var_type_arg =
Variant::new_static(functions::get_dictionary_operator_index()(&arg_dict.godot_owner,
&arg_type_key_variant.native_ptr)); var_type_arg.init_type(variant_type);
...
std::shared_ptr<CPPSignalDescription>
init_signal_description(char *name, std::vector<CPPSignalArg> &args) {
  std::shared_ptr<CPPSignalDescription> description =
      std::make_shared<CPPSignalDescription>();
  ...
  Variant var_name =
  Variant::new_static(functions::get_dictionary_operator_index()(&(*signal_dict_ptr).godot_owner,
  &signal_key_variant.native_ptr));
  ...
  var_args->native_ptr =
  functions::get_dictionary_operator_index()(&(*signal_dict_ptr).godot_owner,
  &arg_key_variant.native_ptr);

  Array* args_array = new Array();
  GDExtensionPtrConstructor constructor =
  functions::get_variant_get_ptr_constructor()(GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_ARRAY,
  0); GDExtensionTypePtr _args[1]; constructor(&args_array->godot_owner,_args);
  ...
  functions::get_array_set_typed()(&args_array->godot_owner,
  GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_DICTIONARY,
  &type_name.godot_owner, &dictionary_var.native_ptr);
