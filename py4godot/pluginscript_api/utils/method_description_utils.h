...
std::shared_ptr<CPPMethodDescription> init_method_description(
    StringName name,
    std::vector<std::shared_ptr<CPPPropertyDescription>> &properties) {
  std::shared_ptr<CPPMethodDescription> res =
      std::make_shared<CPPMethodDescription>();
  res->name = name;
  res->flags = 4096 | 6 | 32768;
  res->id = id++;
  res->args = properties;

  std::shared_ptr<CPPPropertyDescription> return_description =
      std::make_shared<CPPPropertyDescription>();
  return_description->type_ =
