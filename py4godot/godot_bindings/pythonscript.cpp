...
typedef GDExtensionBool (*Py4GodotInitFunc)(
...
initialize_pythonscript(GDExtensionInterfaceGetProcAddress p_get_proc_address,
                        GDExtensionClassLibraryPtr p_library,
                        GDExtensionInitialization *r_initialization) {
  ...
  handle = dlopen((std::string(python_home) + "/bin/main.so").c_str(),
  ...
  void *handle = dlopen((std::string(python_home) + "/bin/main.dylib").c_str(),
                        RTLD_NOW | RTLD_GLOBAL);
#endif
  ...
  Py4GodotInitFunc load_function_handle =
      (Py4GodotInitFunc)dlsym(handle, "py4godot_init");

  // Check for dlsym errors
  const char *dlsym_error = dlerror();
  if (dlsym_error) {
    std::cerr << "Cannot load symbol 'py4godot_init': " << dlsym_error
              << std::endl;
  ...
  int result =
      load_function_handle(p_get_proc_address, p_library, r_initialization);
