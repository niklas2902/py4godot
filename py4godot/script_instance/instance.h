...
GDExtensionBool c_instance_get(GDExtensionScriptInstanceDataPtr p_instance,
  ...
  if (instance->custom_properties.find(std::string{c_method_name}) ==
      instance->custom_properties.end()) {
    // TODO: look over this again. Currently setting editor description is
    // broken.
...
void c_instance_call(GDExtensionScriptInstanceDataPtr p_self,
  ...
  if (is_overridden(p_self, p_method)) {
    MethodCallData data = instance_call(p_self, p_method, p_args,
    ...
    r_error->error =
        GDExtensionCallErrorType::GDEXTENSION_CALL_OK; // signaling that calling
                                                       // the method worked
  }
  PyGILState_Release(gil_state);
}

GDExtensionBool c_instance_set(GDExtensionScriptInstanceDataPtr p_instance,
                               GDExtensionConstStringNamePtr p_name,
  ...
  StringName method_name = StringName::new_static(((void **)p_name)[0]);
  String method_name_str = String::new2(method_name);
  auto length = functions::get_string_to_utf8_chars()(
      &method_name_str.godot_owner, nullptr, 0);
...
create_property_info(std::shared_ptr<CPPPropertyDescription> description_ptr) {
...
std::vector<GDExtensionPropertyInfo> build_property_info(
    std::vector<std::shared_ptr<CPPPropertyDescription>> descriptions) {
...
c_instance_get_property_list(GDExtensionScriptInstanceDataPtr p_instance,
                             uint32_t *r_count) {
  print_error("_c_instance_get_property_list");
  auto p_instance_data = (InstanceData *)p_instance;
  p_instance_data->gd_properties =
      build_property_info(p_instance_data->properties);
...
create_method_info(std::shared_ptr<CPPMethodDescription> description_ptr) {
  ...
  GDExtensionPropertyInfo return_info =
      create_property_info(description_ptr->return_value);
...
c_instance_get_method_list(GDExtensionScriptInstanceDataPtr p_instance,
                           uint32_t *r_count) {
  auto p_instance_data = (InstanceData *)p_instance;
...
c_instance_get_script(GDExtensionScriptInstanceDataPtr p_instance) {
...
GDExtensionBool
c_instance_has_method(GDExtensionScriptInstanceDataPtr p_instance,
                      GDExtensionConstStringNamePtr p_name) {
...
void handle_python_error_instance(PyObject *ptype, PyObject *pvalue,
                                  PyObject *ptraceback) {
...
void init_instance(GDExtensionScriptInstanceInfo *native_script_instance,
                   bool is_placeholder) {
  ...
  native_script_instance->property_can_revert_func =
  instance_property_can_revert; native_script_instance->property_get_revert_func
  ...
  native_script_instance-> refcount_incremented_func =
  instance_ref_count_incremented;
  native_script_instance->refcount_decremented_func =
  instance_ref_count_decremented; native_script_instance->get_script_func =
  instance_get_script; native_script_instance->get_language_func =
  instance_get_language; p_interface->print_error("init_instance3", "test",
  "test",1,1);
  */
}
