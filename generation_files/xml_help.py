...
def format_to_markdown(richtext: str) -> str:
    ...
    replaced_code = replaced_italic.replace(
        "[code]", "`").replace(
        "[/code]", "`")
    replaced_gdscript = re.sub(
        r'\[gdscript\]|\[/gdscript\]',
        lambda m: '```gdscript' if m.group() == '[gdscript]' else '```',
        replaced_code)
    replaced_csharp = re.sub(
        r'\[csharp\].*?\[/csharp\]',
        '',
        replaced_gdscript,
        flags=re.DOTALL)
    replaced_codeblocs = replaced_csharp.replace(
        "[codeblocks]", "").replace(
        "[/codeblocks]", "")
    replaced_codeblock = replaced_codeblocs.replace(
        "[codeblock]", "```gdscript").replace(
        "[/codeblock]", "```")
    replaced_params = re.sub(
        r'\[param ([^\]]+)\]',
        r'`\1`',
        replaced_codeblock)
    replaced_member = re.sub(r'\[member ([^\]]+)\]', r'`\1`', replaced_params)
    replaced_constant = re.sub(
        r'\[constnat ([^\]]+)\]',
        r'`\1`',
        replaced_member)
    replaced_methods = re.sub(
        r'\[method ([^\]]+)\]',
        r'`\1`',
        replaced_constant)
...
def parse_xml(soup: BeautifulSoup) -> XMLDescription:
    ...
    for class_tag in soup.find_all("class"):
        ...
        for prop in class_tag.find_all("property"):
            ...
            property_descriptions[prop_name] = format_to_markdown(
                prop_description)
        ...
        for method in class_tag.find_all("method"):
            ...
            methods_descriptions[method_name] = format_to_markdown(
                method_description)
