...
def format_to_markdown(richtext: str) -> str:
    ...
    replaced_csharp = re.sub(
        r"\[csharp\].*?\[/csharp\]", "", replaced_gdscript, flags=re.DOTALL
    )
    replaced_codeblocs = replaced_csharp.replace("[codeblocks]", "").replace(
        "[/codeblocks]", ""
    )
...
def parse_xml(soup: BeautifulSoup) -> XMLDescription:
    ...
    return XMLDescription(
        class_description, methods_descriptions, property_descriptions
    )
