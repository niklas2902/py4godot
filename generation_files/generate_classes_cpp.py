...
'''
        ...
        res += f'''#include "py4godot/cppclasses/{class_['name']}/
    {class_['name']}.h"'''
        res = generate_newline(res)
        return res
    ...
    if "typedarray" in class_["name"].lower():
        result += f'#include "py4godot/cppclasses/typedarrays/{
            class_["name"]}.h"\n'
    f'#include <memory>"\n'
    return result
...
def generate_constructor_args_array(constructor):
    num_of_constructor_args = len(
        constructor["arguments"]) if "arguments" in constructor.keys() else 1
    ...
    for arg in constructor["arguments"]:
        res += f"{INDENT *
                  2}_args[{index}] = {generate_ret_value_assign_constructor(arg)};"
        res = generate_newline(res)
        index += 1
    res = generate_newline(res)
    return res
...
def convert_camel_case_to_underscore(string):
    ...
    if (("vector4" in res.lower() or "" "vector3" in res.lower() or "vector2" in res.lower())
            or "float64" in res.lower() or "float32" in res.lower() or "int64" in res.lower() or "int32" in res.lower()):
...
def call_constructor_args(constructor):
    ...
    if "arguments" not in constructor.keys():
        return res
    for arg in constructor["arguments"]:
        res += unref_pointer_constructor(
            type_=arg["type"],
            value_name=pythonize_name(
                arg['name'])) + ", "
...
def generate_constructors(class_):
    ...
    for constructor in class_["constructors"]:
        ...
        res = generate_newline(res)
        res += f"{INDENT *
                  2}_class.variant_type = {generate_variant_type(class_['name'])};"
        res = generate_newline(res)
        res += f"{
            INDENT *
            2}GDExtensionPtrConstructor constructor = functions::get_variant_get_ptr_constructor()(_class.variant_type, {
            constructor['index']});"
        res = generate_newline(res)
        ...
        res = generate_newline(res)
        res += f"{INDENT *
                  2}auto _class = {class_['name']}::new{constructor['index']}({call_constructor_args(constructor)});"
        res = generate_newline(res)
...
def generate_make_shared_ptr(class_):
    ...
    if class_["name"] in cpp_core_structs:
        ...
        res = generate_newline(res)
        res += f"{INDENT *
                  3}auto instance_callback = new Callback<{class_['name']}>();"
        res = generate_newline(res)
        res += f"{INDENT *
                  3}auto copy_callback = (Callback<{class_['name']}>*) _class._callback;"
        res = generate_newline(res)
        res += f"{INDENT *
                  3}instance_callback->callback = copy_callback->callback;"
        res = generate_newline(res)
        res += f"{INDENT *
                  3}instance_callback->instance = copy_callback->instance;"
        res = generate_newline(res)
    ...
    res += f"{INDENT *
              2}auto ptr = std::make_shared<{class_['name']}>(_class);"
    res = generate_newline(res)
    res += f"{INDENT * 2}ptr->shouldBeDeleted = false;"
    if class_["name"] in typed_arrays_names or class_[
            "name"] in builtin_classes:
...
def generate_copy_constructor(class_):
    ...
    res = generate_newline(res)
    res += f"{INDENT *
              2}this->variant_type = {get_class_name(generate_variant_type(class_['name']))};"
    res = generate_newline(res)
    ...
    if class_["name"] not in cpp_core_structs:
        res += f"{INDENT *
                  2}_args[0] = &const_cast<{class_['name']}&>(copy_val).godot_owner;"
    ...
    res = generate_newline(res)
    res += f"{INDENT *
              3}auto copy_callback = (Callback<{class_['name']}>*) copy_val._callback;"
    res = generate_newline(res)
    ...
    if "return_value" in method_.keys() or "return_type" in method_.keys():
        ...
        if ret_val.type in classes:
            if ret_val.type in builtin_classes - {"float", "int", "bool", "Nil"} and not is_static(
                ...
                result += f"{INDENT *
                             2}{ret_val.type} {ret_val.name} = {ret_val.type}();"
        elif ret_val.type == "Variant":
            # TODO: is the constructor really necessary
            result += f"{INDENT *
                         2}{ret_val.type} {ret_val.name} = {ret_val.type}(1);"
            ...
            result += f"{INDENT *
                         2}{ungodottype(unbitfield_type(unenumize_type(ret_val.type)))} {ret_val.name};"
    else:
...
def generate_singleton_constructor(classname):
    ...
    res += f"{
        INDENT *
        2}StringName class_name = c_string_to_string_name(\"{
...
def generate_method_bind(current_class, method):
    ...
    res += f"{
        INDENT *
        2}StringName _class_name = c_string_to_string_name(\"{
        get_class_name(
            current_class['name'])}\");"
    res = generate_newline(res)
    ...
    res += f"{INDENT *
              2}StringName _method_name = c_string_to_string_name(\"{method['name']}\");"
    res = generate_newline(res)
...
def generate_method_body_virtual(class_, mMethod):
    ...
    if "return_value" in mMethod.keys():
        res += f"{
            INDENT *
            2}return {
            generate_virtual_return_type(
                unenumize_type(
                    mMethod['return_value']['type']))};"
    ...
    for arg in mMethod["arguments"]:
        ...
        if not arg["type"].startswith("enum::") and not arg["type"].startswith(
            ...
            if arg["type"] in builtin_classes:
                res += f"{INDENT *
                          2}{pythonize_name(arg['name'])} = {arg['type']}::new0()"
            else:
                res += f"{INDENT *
                          2}{pythonize_name(arg['name'])} = {arg['type']}::constructor()"
        res = generate_newline(res)
    return res
...
def generate_varargs_variants(mMethod):
    ...
    for arg in mMethod["arguments"]:
        if arg["type"] in builtin_classes:
            res += f"{
                INDENT *
                2}std::shared_ptr<Variant> variant_{
                pythonize_name(
                    arg['name'])} = std::make_shared<Variant>({
                pythonize_name(
                    arg['name'])});"
        else:
            res += f"{
...
def generate_variant_vector(mMethod):
    ...
    if mMethod["is_vararg"]:
        ...
        res = generate_newline(res)
        res += f"{INDENT *
                  2}std::vector<std::shared_ptr<Variant>> variant_argument_array{{}};"
        res = generate_newline(res)
        if "arguments" in mMethod.keys():
            for arg in mMethod["arguments"]:
                res += f"{
                    INDENT *
                    2}argument_array.push_back(&variant_{
                    pythonize_name(
                        arg['name'])}->native_ptr);"
                res = generate_newline(res)
        ...
        res = generate_newline(res)
        res += f"{INDENT *
                  3}std::shared_ptr<Variant> _var = std::make_shared<Variant>(1);"
        res = generate_newline(res)
        res += f'{INDENT *
                  3}_var->init_from_py_object_native_ptr(_variant, get_python_typename(_variant).c_str());'
        res = generate_newline(res)
...
def generate_delete_varargs_variants(mMethod):
    ...
    res += f"{
        INDENT *
        2}for (auto & vararg: variant_argument_array){
        functions:: get_variant_destroy()( & vararg->native_ptr);} "
    res = generate_newline(res)
    return res
    ...
    for arg in method["arguments"]:
        if arg["type"] == "Variant":
            result += f"{
                ...
                2}std::shared_ptr<Variant> variant_{
                pythonize_name(
                    arg['name'])} = std::make_shared<Variant>(1);"
            result = generate_newline(result)
            result += f"{
                INDENT *
                2}std::string type_name_{
                pythonize_name(
                    arg['name'])} = get_python_typename({
                pythonize_name(
                    arg['name'])});"
            result = generate_newline(result)
            result += f"{
                ...
                2}variant_{
                pythonize_name(
                    arg['name'])}->init_from_py_object_native_ptr({
                pythonize_name(
                    arg['name'])}, type_name_{
                        pythonize_name(
                            arg['name'])}.c_str());"
...
def generate_py_method_body(class_, method):
    ...
    if ("return_value" in method.keys() or "return_type" in method.keys()):
        if not is_static(method):
            result += f"{INDENT *
                         2}auto _ret = this->{pythonize_name(method['name'])}({generate_args_for_call(method)}){convert_to_py_if_variant(method)};"
        else:
            result += f"{INDENT *
                         2}auto _ret = {class_['name']}::{pythonize_name(method['name'])}({generate_args_for_call(method)}){convert_to_py_if_variant(method)};"
    else:
        if not is_static(method):
            result += f"{INDENT *
                         2}this->{pythonize_name(method['name'])}({generate_args_for_call(method)});"
        else:
            result += f"{INDENT *
                         2}{class_['name']}::{pythonize_name(method['name'])}({generate_args_for_call(method)});"
...
def free_variants(mMethod):
    ...
    for argument in mMethod["arguments"]:
        if argument["type"] == "Variant":
            res += f"{
                INDENT *
                2}if (functions::get_variant_get_type()(&variant_{
                pythonize_name(
                    argument['name'])}->native_ptr) != GDEXTENSION_VARIANT_TYPE_OBJECT){{"
            res = generate_newline(res)
            res += f"{
                INDENT *
                3}functions::get_variant_destroy()(&variant_{
                pythonize_name(
                    argument['name'])}->native_ptr);"
            res = generate_newline(res)
            res += f"{INDENT * 2}}}"
            res = generate_newline(res)
    return res
...
def generate_ret_value_assign(argument):
    if argument["type"] in classes - builtin_classes:
        ...
        return f"{
            ...
            pythonize_name(
                argument['name'])}->godot_owner):nullptr"
...
def generate_args_array(method):
    ...
    result = f"{INDENT *
                2}GDExtensionTypePtr _args[{len(method['arguments'])}];"
    result = generate_newline(result)
    for i in range(0, len(method['arguments'])):
        result += f"{
            INDENT *
            2}_args[{i}] = {
            generate_ret_value_assign(
                method['arguments'][i])};"
        result = generate_newline(result)
    return result
...
def generate_callback(class_, method):
    ...
    if "return_value" in method.keys() or "return_type" in method.keys():
        ...
        if is_property_setter(class_, method["name"]) and class_[
                "name"] in builtin_classes:
            result = ""
            result += f"{INDENT * 2}if(_callback != nullptr){{"
            result = generate_newline(result)
            result += f"{INDENT *
                         3}Callback<{ret_val.type}>* _update_callback = (Callback<{ret_val.type}>*)_callback"
            result = generate_newline(result)
            result += f"{
                INDENT *
                3}if(_update_callback) _update_callback->callback(*this, _update_callback->instance);"
            result = generate_newline(result)
            result += f"{INDENT * 2}}}"
            result = generate_newline(result)
            return result
        if is_property_getter(
                class_,
                method["name"]) and ret_val.type in builtin_classes - {
                "float",
                "int",
                "bool",
                "Nil"}:
            setter = get_setter_for_getter(
                class_, method["name"], ret_val.type)
            if setter and "arguments" not in method.keys():
                result = ""
                result += f"{INDENT *
                             2}Callback<{ret_val.type}>* _update_callback = new Callback<{ret_val.type}>();"
                result = generate_newline(result)
                result += f"{
                    INDENT * 2}_update_callback->callback = []({
                    ret_val.type}& arg, void* _instance){{static_cast<{
                    class_['name']}*>(_instance)->{setter}(arg);}};"
                result = generate_newline(result)
                result += f"{INDENT * 2}_update_callback->instance = this;"
                result = generate_newline(result)
                result += f"{INDENT *
                             2}_ret._callback = (Callback<{ret_val.type}>*)_update_callback;"
                result = generate_newline(result)
                return result
    return ""
    ...
    if (class_['name'] in builtin_classes or class_[
        ...
        if class_["name"] in cpp_core_structs:
            if is_static(method):
                result += f"{
                    INDENT *
                    2}method_to_call(nullptr, {
                    get_first_args_native(method)}, {
                    address_ret(method)}, {
                    get_args_count(method)});"
            else:
                result += f"{
                    INDENT *
                    2}method_to_call(&native_struct, {
                    get_first_args_native(method)}, {
                    address_ret(method)}, {
                    get_args_count(method)});"
        else:
...
def generate_method_call_object(method):
    ...
    if not method["is_vararg"]:
        if ("return_value" in method.keys() or "return_type" in method.keys()):
            result += f"{
                INDENT *
                2}functions::get_object_method_bind_ptrcall()(method_bind," f" {
                get_godot_owner(method)}, _args, {
                address_ret(method)});"
        else:
            result += f"{
                INDENT *
        ...
        # p_instance, const GDExtensionConstVariantPtr *p_args, GDExtensionInt
        # p_arg_count, GDExtensionUninitializedVariantPtr r_ret,
        # GDExtensionCallError *r_error
        result += f"{INDENT * 2}GDExtensionCallError call_error;"
        result = generate_newline(result)
        result += f"{
            INDENT *
            2}functions::get_object_method_bind_call()(method_bind," f" {
            get_godot_owner(method)}, &argument_array[0],{
            get_argument_count(method)}, " f"{
                address_ret(method)}, &call_error);"
    return result
...
def generate_destructor(class_):
    ...
    if classname == "RefCounted":
        ...
        res = generate_newline(res)
        res += f"{INDENT *
                  2}if (!already_deleted && godot_owner != nullptr && get_reference_count() == 1 ){{"
        res = generate_newline(res)
        ...
        res = generate_newline(res)
        res += f"{INDENT *
                  2}else if (!already_deleted && godot_owner != nullptr && get_reference_count() > 1){{"
        res = generate_newline(res)
        res += f"{INDENT * 3}unreference();"
        res = generate_newline(res)
        res += f"{INDENT * 3}already_deleted=true;"
        res = generate_newline(res)
        res += f"{INDENT * 2}}}"
    ...
    if classname == "Node":
        ...
        res = generate_newline(res)
        res += f"{INDENT *
                  2}if (!already_deleted && godot_owner != nullptr && !is_inside_tree() && is_put_in_tree){{"
        res = generate_newline(res)
        res += f"{INDENT * 3}functions::get_object_destroy()(godot_owner);"
        res = generate_newline(res)
        res += f"{INDENT * 3}already_deleted=true;"
    ...
    if classname in builtin_classes or classname in typed_arrays_names:
        if class_["has_destructor"]:
            res += f"{
                INDENT *
                2}auto destructor = functions::get_variant_get_ptr_destructor()({
                generate_variant_type(classname)});"
            res = generate_newline(res)
    ...
    res += f"{
        INDENT *
        3}auto destructor = functions::get_variant_get_ptr_destructor()({
        generate_variant_type(classname)});"
    res = generate_newline(res)
...
def extract_arg(type_, index, is_default_value, is_constructor=False):
    ...
    if type_ in classes or "typedarray" in type_.lower():
        if not is_default_value:
            return f"wrapper__extract_ptr_from_{
                untypearray_or_dictionary(type_)}Wrapper(PyTuple_GetItem(args_tuple, {index})) "
        return f"wrapper__default_extract_ptr_from_{
            untypearray_or_dictionary(type_)}Wrapper(PyTuple_GetItem(args_tuple, {index})) "
    elif type_ == "int":
...
def extract_arg_operator(type_):
    ...
    if type_ in classes or "typedarray" in type_.lower():
        return f"wrapper__extract_ptr_from_{
            untypearray_or_dictionary(type_)}Wrapper(wrapper__extract_ptr_from_py_object(other)) "
    elif type_ == "int":
...
def generate_method_switch_args(method, is_constructor=False):
    ...
    if "arguments" in method:
        for arg in method["arguments"]:
            res += extract_arg(arg["type"],
                               index,
                               is_constructor=is_constructor,
                               is_default_value="default_value" in arg) + ", "
            index += 1
...
def generate_switch_methods(class_):
    ...
    res = ""
    res += f"{INDENT}PyObject* {
        class_['name']}::switch_call_return(int method_hash, PyObject* args_tuple){{"
    res = generate_newline(res)
    ...
    for method in methods:
        ...
        if not ("return_value" in method or "return_type" in method):
            res += f"{INDENT *
                      3}case {method_ids['normal_methods'][class_['name']][method['name']]}: {generate_varargs(method)} py_{pythonize_name(method['name'])}({args}); return Py_None;"
            res = generate_newline(res)
            continue
        ...
        if ret_type in {"void*"}:
            ...
            res += f"{INDENT *
                      3}case {method_ids['normal_methods'][class_['name']][method['name']]}: {generate_varargs(method)} return wrapper__create_wrapper_from_{untypearray_or_dictionary(ret_type)}_ptr(py_{pythonize_name(method['name'])}({args}));"
        elif ret_type == "int":
            res += f"{INDENT *
                      3}case {method_ids['normal_methods'][class_['name']][method['name']]}: {generate_varargs(method)} return  PyLong_FromLong(py_{pythonize_name(method['name'])}({args}));"
        elif ret_type == "bool":
            res += f"{INDENT *
                      3}case {method_ids['normal_methods'][class_['name']][method['name']]}: {generate_varargs(method)} return  PyBool_FromLong(py_{pythonize_name(method['name'])}({args}));"
        elif ret_type == "float":
            res += f"{INDENT *
                      3}case {method_ids['normal_methods'][class_['name']][method['name']]}: {generate_varargs(method)} return  PyFloat_FromDouble(py_{pythonize_name(method['name'])}({args}));"
        else:
            res += f"{INDENT *
                      3}case {method_ids['normal_methods'][class_['name']][method['name']]}: {generate_varargs(method)} return py_{pythonize_name(method['name'])}({args});"
        res = generate_newline(res)

    if class_["name"] in builtin_classes:
        res += f"{INDENT *
                  3}case {method_ids['normal_methods'][class_['name']]['py_destroy']}: {class_['name']}_py_destroy();return Py_None;"
        res = generate_newline(res)
    if class_["name"] == "RefCounted":
        res += f"{INDENT *
                  3}case {method_ids['normal_methods'][class_['name']]['py_destroy']}: py_destroy_ref(); return Py_None;"
        res = generate_newline(res)
    ...
    if class_["name"] in core_classes.keys():
        for member in core_classes[class_["name"]].core_members:
            if member.type_ in builtin_classes.union(classes).difference(
                ...
                res += f"{INDENT *
                          3}case {method_ids['normal_methods'][class_['name']]['get_member_' +
                                                                               member.name]}: {generate_varargs(method)} return wrapper__create_wrapper_from_{untypearray_or_dictionary(member.type_)}_ptr(py_member_get_{member.name}());"
                res = generate_newline(res)
            elif member.type_ == "int" or "enum::" in member.type_:
                res += f"{INDENT *
                          3}case {method_ids['normal_methods'][class_['name']]['get_member_' +
                                                                               member.name]}: {generate_varargs(method)} return  PyLong_FromLong(py_member_get_{member.name}());"
            elif member.type_ == "bool":
                res += f"{INDENT *
                          3}case {method_ids['normal_methods'][class_['name']]['get_member_' +
                                                                               member.name]}: {generate_varargs(method)} return  PyBool_FromLong(py_member_get_{member.name}());"
            elif member.type_ == "float" or member.type_ == "double":
                res += f"{INDENT *
                          3}case {method_ids['normal_methods'][class_['name']]['get_member_' +
                                                                               member.name]}: {generate_varargs(method)} return  PyFloat_FromDouble(py_member_get_{member.name}());"
            else:
                res += f"{INDENT *
                          3}case {method_ids['normal_methods'][class_['name']]['get_member_' +
                                                                               member.name]}: return py_member_get_{member.name}();"
            res = generate_newline(res)
            res += f"{INDENT *
                      3}case {method_ids['normal_methods'][class_['name']]['set_member_' +
                                                                           member.name]}: py_member_set_{member.name}({extract_arg(member.type_, 0, is_default_value=False)});return Py_None;"
            res = generate_newline(res)
    if class_["name"] in builtin_classes:
        for operator in operator_dict[get_class_name(class_["name"])]:
            if operator in operator_to_method.keys():
                ...
                res += f"{
                    INDENT *
                    2}case {
                    method_ids['normal_methods'][
                        class_['name']][operator]}: return wrap_operator_{
                    operator_to_python_name(operator)}(PyTuple_GetItem(args_tuple, 0));"
                res = generate_newline(res)

    if "array" in class_["name"].lower():
        res += f"{INDENT *
                  3}case {method_ids['normal_methods'][class_['name']]['__getitem__']}: return py_getitem((int)PyLong_AsLong(PyTuple_GetItem(args_tuple, 0)));break;"
        res = generate_newline(res)
        res += f"{INDENT *
                  3}case {method_ids['normal_methods'][class_['name']]['__setitem__']}: py_setitem((int)PyLong_AsLong(PyTuple_GetItem(args_tuple, 0)), PyTuple_GetItem(args_tuple, 1));return Py_None;"
        res = generate_newline(res)
    if "Dictionary" == class_["name"]:
        res += f"{INDENT *
                  3}case {method_ids['normal_methods'][class_['name']]['__getitem__']}: return py_getitem(PyTuple_GetItem(args_tuple, 0));break;"
        res = generate_newline(res)
        res += f"{INDENT *
                  3}case {method_ids['normal_methods'][class_['name']]['__setitem__']}: py_setitem(PyTuple_GetItem(args_tuple, 0), PyTuple_GetItem(args_tuple, 1));return Py_None;"
        res = generate_newline(res)
    ...
    if "Object" not in class_["name"] and is_refcounted(class_):
        res += f"{INDENT *
                  3}case {method_ids['normal_methods'][class_['name']]['py_destroy']}: py_destroy_ref();return Py_None;"
        res = generate_newline(res)
    ...
    res = generate_newline(res)
    res += f"{INDENT}PyObject* {
        class_['name']}::call_static_method_with_return(int method_hash, PyObject* args_tuple){{"
    res = generate_newline(res)
    res += f"{INDENT}std::vector<PyObject*> varargs{{}};"
    res = generate_newline(res)
    res += f"{INDENT * 2}switch(method_hash){{"
    ...
    for method in static_methods:
        method_id = method_ids['static_methods'][class_[
            'name']][method['name']]
        args = generate_method_switch_args(method)
        if not ("return_value" in method or "return_type" in method):
            res += f"{INDENT *
                      2}case {method_id}: py_{pythonize_name(method['name'])}({args}); break;"
        else:
            ret_type = get_ret_value(method)
            if ret_type in builtin_classes.union(classes).difference(
                    {"float", "int", "bool"}) or "typedarray" in ret_type.lower():
                res += f"{INDENT *
                          3}case {method_id}:{generate_varargs(method)} return wrapper__create_wrapper_from_{untypearray_or_dictionary(ret_type)}_ptr(py_{pythonize_name(method['name'])}({args}));"
            elif ret_type == "int":
                res += f"{INDENT *
                          3}case {method_id}:{generate_varargs(method)} return  PyLong_FromLong(py_{pythonize_name(method['name'])}({args}));"
            elif ret_type == "bool":
                res += f"{INDENT *
                          3}case {method_id}:{generate_varargs(method)} return  PyBool_FromLong(py_{pythonize_name(method['name'])}({args}));"
            elif ret_type == "float":
                res += f"{INDENT *
                          3}case {method_id}:{generate_varargs(method)} return  PyFloat_FromDouble(py_{pythonize_name(method['name'])}({args}));"
            else:
                res += f"{INDENT *
                          3}case {method_id}:{generate_varargs(method)} py_{pythonize_name(method['name'])}({args});break;"
        res = generate_newline(res)
    if class_["name"] in (
        "PackedInt32Array",
        "PackedInt64Array",
        ...
        packed_array_type = {
            ...
            class_['name']]
        method_id = method_ids['static_methods'][class_[
            'name']]["from_memoryview"]
        res += f"{INDENT * 3}case {method_id}:{{"
        res = generate_newline(res)
        res += f"{INDENT * 4}Py_buffer buffer;"
        res = generate_newline(res)
        res += f"{INDENT *
                  4}PyObject* memory_view = PyTuple_GetItem(args_tuple, 0);"
        res = generate_newline(res)
        res += f"{INDENT *
                  4}PyObject_GetBuffer(memory_view, &buffer, PyBUF_SIMPLE);"
        res = generate_newline(res)
        res += f"{INDENT *
                  4}Py_ssize_t n_elements = buffer.len / buffer.itemsize;"
        res = generate_newline(res)
        res += f"{INDENT * 4}void* ptr = buffer.buf;"
        res = generate_newline(res)
        res += f"{
            INDENT *
            4}return wrapper__create_wrapper_from_{
            class_['name']}_ptr({
            class_['name']}::py_from_ptr(({packed_array_type}*)ptr, (long long)n_elements));"
        res = generate_newline(res)
        res += f"{INDENT * 3}}}"
        res = generate_newline(res)
    res += f"{INDENT * 2}}}"
    ...
    res += f"{INDENT}PyObject* godot::{
        class_['name']}::call_constructor(int method_hash, PyObject* args_tuple){{"
    res = generate_newline(res)
    if class_["name"] not in builtin_classes:
        if is_singleton(class_["name"]):
            res += f"{
                INDENT *
                2}return wrapper__create_wrapper_from_{
                class_['name']}_ptr({
                class_['name']}::get_instance());;"
        else:
            res += f"{
                INDENT *
                2}return wrapper__create_wrapper_from_{
                class_['name']}_ptr({
                class_['name']}::constructor());"
    elif class_["name"] in builtin_classes:
        res += f"{INDENT * 2} switch(method_hash){{"
        res = generate_newline(res)
        index = 0
        for constructor in class_["constructors"]:
            args = generate_method_switch_args(
                constructor, is_constructor=True)
            res += f"{INDENT *
                      3}case {index}: return wrapper__create_wrapper_from_{class_['name']}_ptr({class_['name']}::py_new{index}({args}));"
            res = generate_newline(res)
            index += 1
        res = generate_newline(res)
        res += f"{INDENT * 2}}}"
...
def generate_member_getter(class_, member):
    ...
    res += f"{INDENT}{
        make_ptr(
            unbitfield_type(
                unenumize_type(
                    (ungodottype(
                        member.type_)))))} {class_}::py_member_get_{
        member.name}()" + "{"
    res = generate_newline(res)
    ...
    res += f"{INDENT}{
        unbitfield_type(
            unenumize_type(
                (ungodottype(
                    member.type_))))} {class_}::member_get_{
        member.name}()" + "{"
    res = generate_newline(res)
    ...
    res += f'{
        INDENT *
        2}functions::get_string_new_with_utf8_chars()(&_member_name_string.godot_owner, "{
        member.name}");'
    res = generate_newline(res)
    res += f'{INDENT *
              2}StringName _member_name = StringName::new2(_member_name_string);'
    res = generate_newline(res)
    res += f"{
        INDENT *
        2}GDExtensionPtrGetter getter = functions::get_variant_get_ptr_getter()({
        generate_variant_type(class_)},&_member_name.godot_owner);"
    res = generate_newline(res)
    ...
    if member.type_ in builtin_classes - {"int", "float", "bool", "Nil"}:
        ...
        res += f"{
            INDENT * 2}_update_callback->callback = []({
            member.type_}& arg, void* _instance){{static_cast<{class_}*>(_instance)->member_set_{
            member.name}(arg);}};"
        res = generate_newline(res)
        res += f"{INDENT * 2}_update_callback->instance = this;"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}_ret._callback = (Callback<{member.type_}>*)_update_callback;"
        res = generate_newline(res)
    res = generate_newline(res)
...
def generate_ret_ptr(type_, _ret_name="_ret"):
    if untypearray_or_dictionary(type_) in typed_arrays_names:
        return f"{
            untypearray_or_dictionary(type_)}::make_shared_ptr({_ret_name})"
    if type_ in builtin_classes - {"int", "float", "bool", "Nil"}:
...
def generate_member_setter(class_, member):
    ...
    res += f"{
        INDENT *
        2}GDExtensionPtrSetter setter = functions::get_variant_get_ptr_setter()({
        generate_variant_type(class_)},&_member_name.godot_owner);"
    res = generate_newline(res)
    if class_ in cpp_core_structs:
        if member.type_ != "int" and member.type_ != "float" and member.type_ != "double":
    ...
    res = generate_newline(res)
    res += f"{INDENT *
              2}Callback<{class_}>* _update_callback = (Callback<{class_}>*)_callback;"
    res = generate_newline(res)
    res += f"{INDENT *
              2}if(_update_callback) _update_callback->callback(*this, _update_callback->instance);"
    res = generate_newline(res)
    res += "}"
    res = generate_newline(res)
    return res
...
def generate_property(class_, property):
    ...
    if "index" not in property.keys():
        ...
        result += f"{INDENT *
                     2}{simplify_type(unbitfield_type(unenumize_type((property['type']))))} _ret = {pythonize_name(property['getter'])}({property['index']});"
    result = generate_newline(result)
    ...
    if "setter" in property and property["setter"] != "":
        ...
        if "index" not in property.keys():
            result += f"{INDENT *
                         2}{pythonize_name(property['setter'])}(value);"
        else:
            result += f"{INDENT *
                         2}{pythonize_name(property['setter'])}({property['index']},value);"
        result = generate_newline(result)
        result = generate_newline(result)
        if (property["type"] in builtin_classes):
            # TODO: enable this again
        ...
        for operator in operator_dict[get_class_name(class_name)]:
            if operator in operator_to_method.keys():
                ...
                if op.right_type_values:
                    res += f"{INDENT}PyObject* {class_name}::wrap_operator_{
                    ...
                    for return_type in ret_types:
                        ...
                        if right_type != "Variant":
                            ...
                            if ret_type in builtin_classes.union(classes).difference(
                                ...
                                res += f"{INDENT *
                                          2}if (type_checking_{right_type}(other)){{return PyLong_FromLong(py_operator_{operator_to_python_name(operator)}({extract_arg_operator(right_type)}));}}"

                            elif ret_type == "bool":
                                res += f"{INDENT *
                                          2}if (type_checking_{right_type}(other)){{return PyBool_FromLong(py_operator_{operator_to_python_name(operator)}({extract_arg_operator(right_type)}));}}"
                            elif ret_type == "float":
                                res += f"{INDENT *
                                          2}if (type_checking_{right_type}(other)){{return PyFloat_FromDouble(py_operator_{operator_to_python_name(operator)}({extract_arg_operator(right_type)}));}}"
                            else:
                                res += f"{INDENT *
                                          2}if (type_checking_{right_type}(other)){{return py_operator_{operator_to_python_name(operator)}({extract_arg_operator(right_type)});}}"
                            res = generate_newline(res)
                    ...
                    if op.has_variant:
                        ...
                        if ret_type in builtin_classes.union(classes).difference(
                            ...
                            res += f"{
                                INDENT *
                                2}wrapper__create_wrapper_from_{
                                untypearray_or_dictionary(ret_type)}_ptr(py_operator_{
                                operator_to_python_name(operator)}(other));"
                        elif ret_type == "int":
                            res += f"{
                                INDENT *
                                2}return PyLong_FromLong(py_operator_{
                                operator_to_python_name(operator)}(other));"

                        elif ret_type == "bool":
                            res += f"{
                                INDENT *
                                2}return PyBool_FromLong(py_operator_{
                                operator_to_python_name(operator)}(other));"
                        elif ret_type == "float":
                            res += f"{
                                INDENT *
                                2}return PyFloat_FromDouble(py_operator_{
                                operator_to_python_name(operator)}(other));"
                        else:
                            res += f"{
                                INDENT *
                                2}return py_operator_{
                                operator_to_python_name(operator)}(other);"
                        res = generate_newline(res)
                    else:
                        res += f"{INDENT * 2}return Py_None;"
                    ...
                    for return_type in ret_types:
                        ...
                        res = generate_newline(res)
                        res += f"{INDENT *
                                  2}GDExtensionPtrOperatorEvaluator operator_evaluator;"
                        res = generate_newline(res)
                        if right_type in {"float", "int", "bool", "Nil"}:
                            res += f"{INDENT *
                                      2}{ungodottype(right_type)} primitive_val_{right_type} = other;"
                        res = generate_newline(res)
                        res += f"{
                            INDENT *
                            2}operator_evaluator = {
                            INDENT *
                            2}functions::get_variant_get_ptr_operator_evaluator()({
                            operator_to_variant_operator[operator]}, {
                            generate_variant_type(
                                op.class_)}, {
                            generate_variant_type(right_type)});"
                        res = generate_newline(res)
                        if class_name in cpp_core_structs:
                            res += f"{
                                INDENT *
                                2}operator_evaluator(&native_struct, {
                                address_param(right_type)}, {
                                address_ret_decision(
                                    return_type.return_type)});"
                        else:
                            res += f"{
                                INDENT *
                                2}operator_evaluator(&godot_owner, {
                                address_param(right_type)}, {
                                address_ret_decision(
                                    return_type.return_type)});"
                        res = generate_newline(res)
                        ...
                        res = generate_newline(res)
                        res += f"{
                            INDENT * 2}auto _ret = *this {operator} {
                            unref_pointer(
                                type_=ungodottype(right_type),
                                value_name='other')};"
                        res = generate_newline(res)
                        res += f"{INDENT *
                                  2}return {generate_ret_ptr(return_type.return_type)};"
                        res = generate_newline(res)
                        res += INDENT + "}"
                        res = generate_newline(res)
                if op.has_variant:
                    res += f"{INDENT}{
                        op.variant_return_type} {class_name}::operator {operator}(Variant& other)" + "{"
                    res = generate_newline(res)
                    res += f"{INDENT *
                              2}{op.variant_return_type} _ret = {init_return_type(op.variant_return_type)};"
                    res = generate_newline(res)
                    ...
                    if class_name in cpp_core_structs:
                        res += f"{
                            INDENT *
                            2}operator_evaluator(&native_struct, &other.native_ptr, {
                            address_ret_decision(
                                op.variant_return_type)});"
                    else:
                        res += f"{
                            INDENT *
                            2}operator_evaluator(&godot_owner, &other.native_ptr, {
                            address_ret_decision(
                                op.variant_return_type)});"
                    res = generate_newline(res)
                    ...
                    res += f"{INDENT}{
                        make_ptr(
                            op.variant_return_type)} {class_name}::py_operator_{
                        operator_to_python_name(operator)}(PyObject* other)" + "{"
                    res = generate_newline(res)
                    # std::shared_ptr < Variant > _var = std::make_shared < Variant > (1);
                    # _var->init_from_py_object_native_ptr(_variant,
                    # get_python_typename(_variant).c_str());
                    res += f"{INDENT * 2}Variant _var = Variant(1);"
                    res = generate_newline(res)
                    res += f"{
                        INDENT *
                        2}_var.init_from_py_object_native_ptr(other, get_python_typename(other).c_str());"
                    res = generate_newline(res)
                    res += f"{INDENT * 2}auto _ret = *this {operator} _var;"
                    res = generate_newline(res)
                    res += f"{INDENT *
                              2}return {generate_ret_ptr(op.variant_return_type)};"
                    res = generate_newline(res)
                    res += INDENT + "}"
                    res = generate_newline(res)
    res = generate_newline(res)
...
def generate_dictionary_set_item():
    ...
    res = generate_newline(res)
    res += f"{INDENT *
              2}std::shared_ptr<Variant> variant_key = std::make_shared<Variant>(1);"
    res = generate_newline(res)
    res += f"{INDENT *
              2}variant_key->init_from_py_object_native_ptr(key, type_checking__get_name_from_object(key));"
    res = generate_newline(res)
    ...
    res = generate_newline(res)
    res += f"{INDENT *
              2}value_variant.init_from_py_object(value, type_checking__get_name_from_object(value));"
    res = generate_newline(res)
    res += f"{INDENT}" + "}"
    res = generate_newline(res)
...
def generate_dictionary_get_item():
    ...
    res = generate_newline(res)
    res += f"{INDENT *
              2}return Variant::new_static(functions::get_dictionary_operator_index()(&godot_owner, &key.native_ptr));"
    res = generate_newline(res)
    res += f"{INDENT}" + "}"
    res = generate_newline(res)
    return res
...
def generate_array_set_item(class_):
    ...
    if class_["name"] == "PackedInt32Array":
        ...
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return functions::get_packed_int32_array_operator_index()(&godot_owner, index)[0];"
        res += INDENT + "}"
    elif class_["name"] == "PackedInt64Array":
        res += f"{INDENT}int64_t& {
            class_['name']}::operator[](int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return functions::get_packed_int64_array_operator_index()(&godot_owner, index)[0];"
        res += INDENT + "}"
    elif class_["name"] == "PackedFloat32Array":
        res += f"{INDENT}float& {class_['name']}::operator[](int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return functions::get_packed_float32_array_operator_index()(&godot_owner, index)[0];"
        res += INDENT + "}"
    elif class_["name"] == "PackedFloat64Array":
        res += f"{INDENT}double& {class_['name']}::operator[](int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return functions::get_packed_float64_array_operator_index()(&godot_owner, index)[0];"
        res += INDENT + "}"
    elif class_["name"] == "PackedByteArray":
        res += f"{INDENT}byte& {class_['name']}::operator[](int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return functions::get_packed_byte_array_operator_index()(&godot_owner, index)[0];"
        res += INDENT + "}"

    elif class_["name"] == "PackedColorArray":
        res += f"{INDENT}std::shared_ptr<Color> {
            class_['name']}::operator[](int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return std::make_shared<Color>(Color::new_static(functions::get_packed_color_array_operator_index()(&godot_owner, index)));"
        res += INDENT + "}"
    elif class_["name"] == "PackedVector3Array":
        res += f"{INDENT}std::shared_ptr<Vector3> {
            class_['name']}::operator[](int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return std::make_shared<Vector3>(Vector3::new_static(functions::get_packed_vector3_array_operator_index()(&godot_owner, index)));"
        res += INDENT + "}"
    elif class_["name"] == "PackedVector2Array":
        res += f"{INDENT}std::shared_ptr<Vector2> {
            class_['name']}::operator[](int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return std::make_shared<Vector2>(Vector2::new_static(functions::get_packed_vector2_array_operator_index()(&godot_owner, index)));"
        res += INDENT + "}"
    elif class_["name"] == "PackedStringArray":
        res += f"{INDENT}std::shared_ptr<String> {
            class_['name']}::operator[](int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return std::make_shared<String>(String::new_static(functions::get_packed_string_array_operator_index()(&godot_owner, index)));"
        res += INDENT + "}"
    elif get_class_name(class_["name"]) == "Array":
        res += f"{INDENT}Variant {class_['name']}::operator[](int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return Variant::new_static(functions::get_array_operator_index()(&godot_owner, index));"
        res += INDENT + "}"
    return res
...
def generate_array_get_item_wrapper(class_):
    ...
    if class_["name"] == "PackedInt32Array":
        res += f"{INDENT}PyObject* {
            class_['name']}::py_getitem(int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return PyLong_FromLong(functions::get_packed_int32_array_operator_index()(&godot_owner, index)[0]);"
        res += INDENT + "}"
    elif class_["name"] == "PackedInt64Array":
        res += f"{INDENT}PyObject* {
            class_['name']}::py_getitem(int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return PyLong_FromLong(functions::get_packed_int64_array_operator_index()(&godot_owner, index)[0]);"
        res += INDENT + "}"
    elif class_["name"] == "PackedFloat32Array":
        res += f"{INDENT}PyObject* {
            class_['name']}::py_getitem(int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return PyFloat_FromDouble(functions::get_packed_float32_array_operator_index()(&godot_owner, index)[0]);"
        res += INDENT + "}"
    elif class_["name"] == "PackedFloat64Array":
        res += f"{INDENT}PyObject* {
            class_['name']}::py_getitem(int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return PyFloat_FromDouble(functions::get_packed_float64_array_operator_index()(&godot_owner, index)[0]);"
        res += INDENT + "}"
    elif class_["name"] == "PackedByteArray":
        res += f"{INDENT}PyObject* {
            class_['name']}::py_getitem(int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return PyLong_FromLong(functions::get_packed_byte_array_operator_index()(&godot_owner, index)[0]);"
        res += INDENT + "}"
        ...
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return wrapper__create_wrapper_from_Color_ptr(std::make_shared<Color>(Color::new_static(((void**)functions::get_packed_color_array_operator_index()(&godot_owner, index))[0])));"
        res += INDENT + "}"
    elif class_["name"] == "PackedVector3Array":
        res += f"{INDENT}PyObject* {
            class_['name']}::py_getitem(int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return wrapper__create_wrapper_from_Vector3_ptr(std::make_shared<Vector3>(Vector3::new_static(((void**)functions::get_packed_vector3_array_operator_index()(&godot_owner, index))[0])));"
        res += INDENT + "}"
    elif class_["name"] == "PackedVector2Array":
        res += f"{INDENT}PyObject* {
            class_['name']}::py_getitem(int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return wrapper__create_wrapper_from_Vector2_ptr(std::make_shared<Vector2>(Vector2::new_static(((void**)functions::get_packed_vector2_array_operator_index()(&godot_owner, index))[0])));"
        res += INDENT + "}"
    elif class_["name"] == "PackedStringArray":
        res += f"{INDENT}PyObject* {
            class_['name']}::py_getitem(int index)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}return wrapper__create_wrapper_from_String_ptr(std::make_shared<String>(String::new_static(((void**)functions::get_packed_string_array_operator_index()(&godot_owner, index))[0])));"
        res += INDENT + "}"
    elif get_class_name(class_["name"]) == "Array":
        res += f"{INDENT}PyObject* {
            class_['name']}::py_getitem(int index)" + "{"
        ...
        res = generate_newline(res)
        res += f"{INDENT *
                  2}variant.native_ptr = (functions::get_array_operator_index()(&godot_owner, index));"
        res = generate_newline(res)
        res += f"{INDENT * 2}return variant.get_converted_value(true);"
        res += INDENT + "}"
    else:
...
def generate_array_set_item_wrapper(class_):
    ...
    if class_["name"] == "PackedInt32Array":
        res += f"{INDENT}void {
            class_['name']}::py_setitem(int index,PyObject* value)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}functions::get_packed_int32_array_operator_index()(&godot_owner, index)[0] = (int32_t)(PyLong_AsLong(value));"
        res += INDENT + "}"
    elif class_["name"] == "PackedInt64Array":
        res += f"{INDENT}void {
            class_['name']}::py_setitem(int index,PyObject* value)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}functions::get_packed_int64_array_operator_index()(&godot_owner, index)[0] = (int64_t)(PyLong_AsLong(value));"
        res += INDENT + "}"
    elif class_["name"] == "PackedFloat32Array":
        res += f"{INDENT}void {
            class_['name']}::py_setitem(int index,PyObject* value)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}functions::get_packed_float32_array_operator_index()(&godot_owner, index)[0] = (float)(PyFloat_AsDouble(value));"
        res += INDENT + "}"
    elif class_["name"] == "PackedFloat64Array":
        res += f"{INDENT}void {
            class_['name']}::py_setitem(int index,PyObject* value)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}functions::get_packed_float64_array_operator_index()(&godot_owner, index)[0] = (PyFloat_AsDouble(value));"
        res += INDENT + "}"
    elif class_["name"] == "PackedByteArray":
        res += f"{INDENT}void {
            class_['name']}::py_setitem(int index,PyObject* value)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}functions::get_packed_byte_array_operator_index()(&godot_owner, index)[0] = (byte)(PyLong_AsLong(value));"
        res += INDENT + "}"
        ...
        res = generate_newline(res)
        res += f"{INDENT *
                  2}((void**)functions::get_packed_color_array_operator_index()(&godot_owner, index))[0] = wrapper__extract_ptr_from_ColorWrapper(value)->godot_owner;"
        res += INDENT + "}"
    elif class_["name"] == "PackedVector3Array":
        res += f"{INDENT}void {
            class_['name']}::py_setitem(int index,PyObject* value)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}((void**)functions::get_packed_vector3_array_operator_index()(&godot_owner, index))[0] = wrapper__extract_ptr_from_Vector3Wrapper(value)->godot_owner;"
        res += INDENT + "}"
    elif class_["name"] == "PackedVector2Array":
        res += f"{INDENT}void {
            class_['name']}::py_setitem(int index,PyObject* value)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2}((void**)functions::get_packed_vector2_array_operator_index()(&godot_owner, index))[0] = wrapper__extract_ptr_from_Vector2Wrapper(value)->godot_owner;"
        res += INDENT + "}"
    elif class_["name"] == "PackedStringArray":
        res += f"{INDENT}void {
            class_['name']}::py_setitem(int index,PyObject* value)" + "{"
        res = generate_newline(res)
        res += f"{INDENT *
                  2} ((void**)functions::get_packed_string_array_operator_index()(&godot_owner, index))[0] = wrapper__extract_ptr_from_StringWrapper(value)->godot_owner;"
        res += INDENT + "}"
    elif get_class_name(class_["name"]) == "Array":
        res += f"{INDENT}void {
            class_['name']}::py_setitem(int index,PyObject* value)" + "{"
        ...
        res = generate_newline(res)
        res += f"{INDENT *
                  2}variant.init_from_py_object(value, type_checking__get_name_from_object(value));"
        res += INDENT + "}"
...
def generate_special_methods_packed_array(class_):
    res = ""
    method = {
        "PackedInt32Array": "functions::get_packed_int32_array_operator_index()",
        ...
        "PackedByteArray": "functions::get_packed_byte_array_operator_index()"}[
        class_['name']]
    format = {
    ...
    res = generate_newline(res)
    res += f"{INDENT *
              1}std::vector<{packed_array_type}> {class_['name']}::to_vector(){{"
    res = generate_newline(res)
    res += f"{INDENT * 2}{packed_array_type}* ptr = {method}(&godot_owner, 0);"
    res = generate_newline(res)
    res += f"{INDENT * 2}long long length = size();"
    res = generate_newline(res)
    res += f"{INDENT *
              2}std::vector<{packed_array_type}> result = std::vector<{packed_array_type}>(ptr, ptr + length);"
    res = generate_newline(res)
    res += f"{INDENT * 2}return result;}}"
    res = generate_newline(res)

    res += f"{INDENT *
              1}{packed_array_type}* {class_['name']}::get_pointer(){{"
    res = generate_newline(res)
    res += f"{INDENT * 2}{packed_array_type}* ptr = {method}(&godot_owner, 0);"
    res = generate_newline(res)
    res += f"{INDENT * 2}return ptr;"
    ...
    res += f"{
        ...
        1}std::shared_ptr<{
        class_['name']}> {
            class_['name']}::py_from_ptr({packed_array_type}* ptr, long long size){{"
    res = generate_newline(res)
    res = generate_newline(res)
    res += f"{INDENT * 2}{class_['name']} _class{{}};"
    res = generate_newline(res)
    ...
    res = generate_newline(res)
    res += f"{INDENT *
              2}{packed_array_type}* packed_array_ptr = {method}(&_class.godot_owner, 0);"
    res = generate_newline(res)
    ...
    if ("properties" in class_.keys()):
        for property in class_["properties"]:
            if "setter" in property.keys():
                if methodname == property["getter"]:
                    for method in class_["methods"]:
                        if method["name"] == property["setter"] and are_types_same(
                                method, ret_type):
...
if __name__ == "__main__":
    ...
    with open('py4godot/gdextension-api/extension_api.json', 'r', encoding="utf-8") as myfile:
        ...
        for class_ in obj["classes"]:
            ...
            generate_classes(
                [class_], f"py4godot/cppclasses/{class_['name']}/{class_['name']}.cpp")
        if not os.path.exists(f"py4godot/cppclasses/typedarrays/"):
            os.mkdir(f"py4godot/cppclasses/typedarrays/")
        for array in arrays:
            generate_classes(
                [array], f"py4godot/cppclasses/typedarrays/{array['name']}.cpp", is_core=True)
        generate_classes(
            obj["builtin_classes"],
            f"py4godot/cppclasses/generated4_core.cpp",
            is_core=True)
