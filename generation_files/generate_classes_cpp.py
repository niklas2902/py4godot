...
from generate_classes_hpp import (
    collect_class_structs,
    get_ret_value,
    has_native_struct,
    ungodottype,
)
...
def generate_args(
    method_with_args, builtin_classes, is_cpp=False, should_make_shared=False
):
    result = " "
    ...
    for arg in method_with_args["arguments"]:
        if (
            arg["type"] not in builtin_classes
            and not arg["type"].startswith("enum::")
            and not arg["type"].startswith("bitfield::")
            and not arg["type"].startswith("typedarray::")
...
def generate_constructor_args_array(constructor):
    num_of_constructor_args = (
        len(constructor["arguments"]) if "arguments" in constructor.keys() else 1
    )
...
def convert_camel_case_to_underscore(string):
    ...
    if (
        (
            "vector4" in res.lower()
            or "" "vector3" in res.lower()
            or "vector2" in res.lower()
        )
...
def call_constructor_args(constructor):
    ...
    for arg in constructor["arguments"]:
        res += (
            unref_pointer_constructor(
                type_=arg["type"], value_name=pythonize_name(arg["name"])
            )
            + ", "
        )
...
def generate_constructors(class_):
    ...
    for constructor in class_["constructors"]:
        res += (
            f"{INDENT}{class_['name']} {class_['name']}::new{constructor['index']}({generate_constructor_args(constructor)})"
            + "{"
        )
        res = generate_newline(res)
        ...
        res += (
            f"{INDENT}std::shared_ptr<{class_['name']}> {class_['name']}::py_new{constructor['index']}({generate_constructor_args(constructor, True)})"
            + "{"
        )
        res = generate_newline(res)
...
def generate_copy_constructor(class_):
    res = ""
    res += (
        f"{INDENT}{class_['name']}::{class_['name']}(const {class_['name']}& copy_val)"
        + "{"
    )
    res = generate_newline(res)
    res = generate_newline(res)
...
def generate_copy_operator(class_):
    res = ""
    res += (
        f"{INDENT}{class_['name']}& {class_['name']}::operator=(const {class_['name']}& copy_val)"
        + "{"
    )
    res = generate_newline(res)
    res = generate_newline(res)
    ...
    if "return_value" in method_.keys() or "return_type" in method_.keys():
        ...
        if ret_val.type in classes:
            if ret_val.type in builtin_classes - {
                ...
                method_
            ):  # TODO get rid of is_static
                result += f"{INDENT * 2}{ret_val.type} {ret_val.name}{{}};"
            elif ret_val.type in builtin_classes:
                result += (
                    f"{INDENT * 2}{ungodottype(ret_val.type)} {ret_val.name}" + "{};"
                )
            else:
                result += (
                    f"{INDENT * 2}{ret_val.type} {ret_val.name} = {ret_val.type}();"
                )
...
def generate_method_bind(current_class, method):
    ...
    res = generate_newline(res)
    if (
        current_class["name"] in builtin_classes
        or current_class["name"] in typed_arrays_names
    ):
        res += (
            f"""{INDENT * 2}GDExtensionPtrBuiltInMethod method_to_call = """
        ...
        res += (
            f"""{INDENT * 2}GDExtensionMethodBindPtr method_bind = """
    ...
    for arg in mMethod["arguments"]:
        ...
        if (
            ...
            if arg["type"] in builtin_classes:
                res += (
                    f"{INDENT * 2}{pythonize_name(arg['name'])} = {arg['type']}::new0()"
                )
            else:
...
def generate_variant_vector(mMethod):
    ...
    if mMethod["is_vararg"]:
        ...
        res = generate_newline(res)
        res += (
            f"{INDENT * 3}std::shared_ptr<Variant> _var = std::make_shared<Variant>(1);"
        )
        res = generate_newline(res)
...
def generate_method(class_, mMethod):
    ...
    py_def_function = (
        f"{INDENT}{make_ptr(unvarianttype(ungodottype(unenumize_type(untypearray_or_dictionary(get_ret_value(mMethod))))))} {class_['name']}::py_{pythonize_name(mMethod['name'])}({args_ptr})"
        + "{"
    )
    res += py_def_function
    res = generate_newline(res)
    ...
    def_function = (
        f"{INDENT}{ungodottype(unenumize_type(untypearray_or_dictionary(get_ret_value(mMethod))))} {class_['name']}::{pythonize_name(mMethod['name'])}({args})"
        + "{"
    )
    res += def_function
    res = generate_newline(res)
...
def generate_callback(class_, method):
    ...
    if "return_value" in method.keys() or "return_type" in method.keys():
        ...
        if (
            is_property_setter(class_, method["name"])
        ...
        if is_property_getter(
            class_, method["name"]
        ) and ret_val.type in builtin_classes - {"float", "int", "bool", "Nil"}:
...
def generate_method_call_object(method):
    ...
    if not method["is_vararg"]:
        if "return_value" in method.keys() or "return_type" in method.keys():
            result += (
                f"{INDENT * 2}functions::get_object_method_bind_ptrcall()(method_bind,"
        ...
        result = generate_newline(result)
        result += (
            f"{INDENT * 2}functions::get_object_method_bind_call()(method_bind,"
...
def generate_destructor(class_):
    ...
    if classname == "RefCounted":
        ...
        res += f"{INDENT * 2}}}"
        res = generate_newline(res)
        res += f"{INDENT*2}else if (!already_deleted && godot_owner != nullptr && get_reference_count() > 1){{"
        res = generate_newline(res)
...
def extract_arg(type_, index, is_default_value, is_constructor=False):
    ...
    if type_ in classes or "typedarray" in type_.lower():
        ...
        return (
            f"(PyObject_IsTrue(PyTuple_GetItem(args_tuple, {index})) ? true : false) "
        )
...
def collect_methods(class_, static_methods):
    ...
    if "methods" in class_:
        res += list(
            filter(
                lambda method: is_static(method) == static_methods
                and not has_native_struct(method),
                class_["methods"],
            )
        )
...
def generate_switch_methods(class_):
    ...
    for method in methods:
        ...
        if ret_type in {"void*"}:
            ...
            ret_type
            in builtin_classes.union(classes).difference({"float", "int", "bool"})
            or "typedarray" in ret_type.lower()
        ):
    ...
    if class_["name"] in core_classes.keys():
        for member in core_classes[class_["name"]].core_members:
            if (
                member.type_
                in builtin_classes.union(classes).difference({"float", "int", "bool"})
                or "typedarray" in member.type_.lower()
            ):
...
def generate_member_getter(class_, member):
    res = ""
    res += (
        f"{INDENT}{make_ptr(unbitfield_type(unenumize_type((ungodottype(member.type_)))))} {class_}::py_member_get_{member.name}()"
        + "{"
    )
    res = generate_newline(res)
    ...
    res += (
        f"{INDENT}{unbitfield_type(unenumize_type((ungodottype(member.type_))))} {class_}::member_get_{member.name}()"
        + "{"
    )
    res = generate_newline(res)
    ...
    res = generate_newline(res)
    res += (
        f"{INDENT * 2}StringName _member_name = StringName::new2(_member_name_string);"
    )
    res = generate_newline(res)
    ...
    if class_ in cpp_core_structs:
        if (
            member.type_ != "int"
            and member.type_ != "float"
            and member.type_ != "double"
        ):
    ...
    if member.type_ in builtin_classes - {"int", "float", "bool", "Nil"}:
        ...
        res = generate_newline(res)
        res += (
            f"{INDENT * 2}_ret._callback = (Callback<{member.type_}>*)_update_callback;"
        )
        res = generate_newline(res)
    res = generate_newline(res)
...
def generate_member_setter(class_, member):
    res = ""
    res += (
        f"{INDENT}void {class_}::py_member_set_{member.name}({make_ptr(member.type_)} value)"
        + "{"
    )
    res = generate_newline(res)
    ...
    res += (
        f"{INDENT}void {class_}::member_set_{member.name}({member.type_}& value)" + "{"
    )
    res = generate_newline(res)
    res += f"{INDENT * 2}String _member_name_string = String::new0();"
    res = generate_newline(res)
...
def generate_property(class_, property):
    ...
    result = generate_newline(result)
    result += (
        f"{INDENT}{simplify_type(untypearray_or_dictionary(unbitfield_type(unenumize_type((property['type'])))))} {class_['name']}::prop_get_{pythonize_name(property['name'])}()"
        + "{"
    )
    result = generate_newline(result)
    ...
    if "setter" in property and property["setter"] != "":
        result += (
            f"{INDENT}void {class_['name']}::prop_set_{pythonize_name(property['name'])}({untypearray_or_dictionary(simplify_type(property['type']))} value)"
            + "{"
        )
        result = generate_newline(result)
        if "index" not in property.keys():
...
def generate_new_static(class_):
    ...
    if class_["name"] in builtin_classes:
        res += (
            f"{INDENT}{class_['name']} {class_['name']}::new_static(GDExtensionTypePtr owner)"
            + "{"
        )
    else:
        res += (
            f"{INDENT}{class_['name']} {class_['name']}::new_static(GDExtensionObjectPtr owner)"
            + "{"
        )
...
def generate_set_owner(class_):
    ...
    if class_["name"] in builtin_classes:
        res += (
            f"{INDENT}void {class_['name']}::set_gdowner_{class_['name']}(GDExtensionTypePtr owner)"
            + "{"
        )
    else:
        res += (
            f"{INDENT}void {class_['name']}::set_gdowner_{class_['name']}(GDExtensionObjectPtr owner)"
            + "{"
        )
    ...
    if get_class_name(class_name) in operator_dict.keys():
        for operator in operator_dict[get_class_name(class_name)]:
            if operator in operator_to_method.keys():
                ...
                if op.right_type_values:
                    ...
                    for return_type in ret_types:
                        ...
                        if right_type != "Variant":
                            ...
                            if (
                                ret_type
                                in builtin_classes.union(classes).difference(
                                    {"float", "int", "bool"}
                                )
                                or "typedarray" in ret_type.lower()
                            ):
                    ...
                    for return_type in ret_types:
                        right_type = return_type.right_type
                        res += (
                            f"{INDENT}{return_type.return_type} {class_name}::operator {operator}({ungodottype(right_type)}{generate_reference(right_type)} other)"
                            + "{"
                        )
                        res = generate_newline(res)
                        ...
                        res += (
                            f"{INDENT}{make_ptr(return_type.return_type)} {class_name}::py_operator_{operator_to_python_name(operator)}({make_ptr(ungodottype(right_type))} other)"
                            + "{"
                        )
                        res = generate_newline(res)
                ...
                if op.has_variant:
                    res += (
                        f"{INDENT}{op.variant_return_type} {class_name}::operator {operator}(Variant& other)"
                        + "{"
                    )
                    res = generate_newline(res)
                    ...
                    res += (
                        f"{INDENT}{make_ptr(op.variant_return_type)} {class_name}::py_operator_{operator_to_python_name(operator)}(PyObject* other)"
                        + "{"
                    )
                    res = generate_newline(res)
...
def generate_array_set_item(class_):
    ...
    if class_["name"] == "PackedInt32Array":
        ...
        res += (
            f"{INDENT}std::shared_ptr<Color> {class_['name']}::operator[](int index)"
            + "{"
        )
        res = generate_newline(res)
        ...
        res += (
            f"{INDENT}std::shared_ptr<Vector3> {class_['name']}::operator[](int index)"
            + "{"
        )
        res = generate_newline(res)
        ...
        res += (
            f"{INDENT}std::shared_ptr<Vector2> {class_['name']}::operator[](int index)"
            + "{"
        )
        res = generate_newline(res)
        ...
        res += (
            f"{INDENT}std::shared_ptr<String> {class_['name']}::operator[](int index)"
            + "{"
        )
        res = generate_newline(res)
...
def generate_array_set_item_wrapper(class_):
    ...
    if class_["name"] == "PackedInt32Array":
        res += (
            f"{INDENT}void {class_['name']}::py_setitem(int index,PyObject* value)"
            + "{"
        )
        res = generate_newline(res)
...
def generate_special_methods_packed_array(class_):
    ...
    res = generate_newline(res)
    res += (
        f"{INDENT * 1}std::vector<{packed_array_type}> {class_['name']}::to_vector(){{"
    )
    res = generate_newline(res)
    res += f"{INDENT*2}{packed_array_type}* ptr = {method}(&godot_owner, 0);"
    res = generate_newline(res)
    ...
    for operator in class_["operators"]:
        ...
        if "right_type" in operator.keys():
            if operator["right_type"] == "Variant":
                ...
                operator_dict[class_["name"]][operator["name"]].variant_return_type = (
...
if __name__ == "__main__":
    ...
    with open(
        ...
        for array in arrays:
            generate_classes(
                [array],
                f"py4godot/cppclasses/typedarrays/{array['name']}.cpp",
                is_core=True,
            )
        generate_classes(
            obj["builtin_classes"],
