...
from generation_files.utils.constant_generation_helpers import (
    generate_constants_for_class,
    ...
    for arg in mMethod["arguments"]:
        ...
        if (
            ...
            if arg["type"] in builtin_classes:
                ...
                type_ = arg["type"]
                res += (
                    f"{INDENT * 3}{pythonize_name(arg['name'])} = c_utils.empty_object"
                )
        res = generate_newline(res)
    return res
...
def generate_method_args(class_, method):
    ...
    for arg in method["arguments"]:
        if (
            ...
            res += (
                f"{pythonize_name(arg['name'])}._ptr, "  # Todo: implement conditional
            )
        elif (
...
def generate_init_signals(cls):
    ...
    if (
        cls["name"] != "Object"
        and cls["name"] not in builtin_classes
        and not "typedarray" in cls["name"].lower()
    ):
...
def generate_member_setter(class_, member):
    ...
    res += (
        f"{INDENT}def set_{member.name}(self, value: '{undouble_type(member.type_)}' ):"
    )
    res = generate_newline(res)
    res += body
    return res
...
def generate_children(containing_class_, type_):
    ...
    for cls in already_registed_classes:
        res += (
            ungodottype_type_array(
                unbitfield_type(unenumize_type(cls)), containing_class_
            )
            + ","
        )
    return res[:-1] + "]"
...
def generate_property(property, classname):
    ...
    if "setter" in property and property["setter"] != "":
        ...
        if classname == "RichTextLabel" and property["name"] == "custom_effects":
            ...
            result += (
                f"{INDENT}def {pythonize_name(property['name'])}(self, value:'Array'):"
            )
        else:
...
def generate_args(class_, method_with_args):
    ...
    for arg in method_with_args["arguments"]:
        if not arg["type"].startswith("enum::"):
            ...
            arg_type = arg["type"].replace("enum::", "")
            type_ = unstring(
                unvariant(untypearray_or_dictionary(unenumize_type(arg_type)))
            )
            if class_["name"] in typed_arrays_names:
                type_ = unstring(
                    unvariant_type_array(
                        untypearray_or_dictionary(unenumize_type(arg_type)),
                        class_["name"],
                    )
                )
            ...
            if operator in operator_to_method.keys():
                ...
                for return_type in op.return_types:
                    if return_type.right_type:
                        res += (
                            f"{INDENT*2}if isinstance(other, {return_type.right_type}):"
                        )
                        res = generate_newline(res)
...
def generate_classes(classes, filename, is_core=False, is_typed_array=False):
    ...
    if is_typed_array:
        ...
        for cls in classes_to_import:
            if cls in [class_["name"] for class_ in classes]:
                continue
            if cls in [cls["name"] for cls in obj["classes"]] and cls in [
        ...
        for cls in classes_to_import:
            if cls in [class_["name"] for class_ in classes]:
                continue
            if cls in [cls["name"] for cls in obj["classes"]] and cls not in [
                class_["inherits"] for class_ in classes
            ]:
                continue
            res += f"import py4godot.classes.{cls} as py4godot_{cls.lower()} "
    ...
    for operator in class_["operators"]:
        ...
        if "right_type" in operator.keys():
            if operator["right_type"] == "Variant":
                ...
                operator_dict[class_["name"]][operator["name"]].variant_return_value = (
...
if __name__ == "__main__":
    ...
    with open("py4godot/gdextension-api/extension_api.json", "r") as myfile:
        ...
        generate_classes(
            obj["builtin_classes"], f"py4godot/classes/core.py", is_core=True
        )
