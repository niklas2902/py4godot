    ...
    for arg in args:
        if arg["type"] == "StringName":
            res += f"{
                ...
                2}assert(isinstance({
                pythonize_name(
                    arg['name'])}, (str, unicode, StringName)))"
            res = generate_newline(res)
            res += f"{
                INDENT *
                2}cdef StringName py_stringname_{
                pythonize_name(
                    arg['name'])} = {
                pythonize_name(
                    arg['name'])} if isinstance({
                        pythonize_name(
                            arg['name'])}, StringName) else StringName.new2({
                                pythonize_name(
                                    arg['name'])})"
            ...
            res += f"{
                ...
                2}assert(isinstance({
                pythonize_name(
                    arg['name'])}, (str, unicode, NodePath)))"
            res = generate_newline(res)
            res += f"{
                INDENT *
                2}cdef NodePath py_nodepath_{
                pythonize_name(
                    arg['name'])} = {
                pythonize_name(
                    arg['name'])} if isinstance({
...
def generate_constructors(class_):
    ...
    for constructor in class_["constructors"]:
        ...
        if "arguments" in constructor:
            ...
            res = generate_newline(res)
        res += f"{INDENT *
                  2}_class.{class_['name']}_internal_class_ptr = (CPP{class_['name']}.py_new{constructor['index']}({generate_constructor_call_args(class_, constructor)}))"
        res = generate_newline(res)
...
def generate_return_value(classname, method_):
    ...
    if "return_value" in method_.keys() or "return_type" in method_.keys():
        ...
        if ret_val.type in {"int", "float", "bool"}:
            result += f"{INDENT *
                         2}cdef {ungodottype(ret_val.type)} {ret_val.name} = 0"
        elif ret_val.type in classes:
            if ret_val.type in builtin_classes.union(
                    {"Object"}) or ret_val.type == classname:
                result += f"{
                    INDENT *
                    2}cdef {
                    ret_val.type} {
                    ret_val.name} = {
                    ret_val.type}.__new__({
                    ret_val.type})"
            else:
                result += f"{
                    INDENT *
                    ...
                    ret_val.type.lower()}.{
                    ret_val.type} {
                    ret_val.name} = py4godot_{
                    ret_val.type.lower()}.{
                        ret_val.type}.__new__(py4godot_{
                            ret_val.type.lower()}.{
                                ret_val.type})"
            ...
            result += (
                f"{
                    ...
                    generate_typed_array_name(
                        ret_val.type).lower()}.{
                    generate_typed_array_name(
                        ret_val.type)} _ret = " f"py4godot_{
                            generate_typed_array_name(
                                ret_val.type).lower()}.{
                                    generate_typed_array_name(
                                        ret_val.type)}.__new__(" f"py4godot_{
                                            generate_typed_array_name(
                                                ret_val.type).lower()}.{
                                                    generate_typed_array_name(
                                                        ret_val.type)})")
    ...
    if "return_value" in method_.keys() or "return_type" in method_.keys():
        ...
        if ret_val.type == "String":
            ...
            if ret_val.type in classes - builtin_classes:
                result = f"{
                    INDENT *
                    2}return None if _ret.{
                    ret_val.type}_internal_class_ptr.get().get_godot_owner() == NULL else smart_cast(_ret)"
            else:
                result = ""
...
def generate_native_params(mMethod):
    ...
    for arg in mMethod["arguments"]:
        if arg["type"] == "String":
            res += f"{
                INDENT *
                2}cdef String string_{
                arg['name']} = c_string_to_string({
                pythonize_name(
                    arg['name'])}.encode('utf-8'))"
            res = generate_newline(res)
        if arg["type"] == "StringName":
            res += f"{
                INDENT *
                2}cdef StringName string_name_{
                arg['name']} = c_string_to_string_name({
                pythonize_name(
                    arg['name'])}.encode('utf-8'))"
            res = generate_newline(res)
        if arg["type"] == "Variant":
    ...
    for arg in mMethod["arguments"]:
        ...
        if not arg["type"].startswith("enum::") and not arg["type"].startswith(
            ...
            if arg["type"] in builtin_classes:
                ...
                type_ = arg["type"]
                res += f"{INDENT *
                          3}{pythonize_name(arg['name'])} = py4godot_{type_.lower()}.{type_}.constructor()"
        res = generate_newline(res)
    return res
...
def generate_ret_call(method_):
    ...
    if "return_value" in method_.keys() or "return_type" in method_.keys():
        ...
        if ret_val.type in classes:
            ...
            result += f"_ret.{
                untypearray_or_dictionary(
                    ret_val.type)}_internal_class_ptr"
        else:
            result += f"_ret"
    else:
        result += f"_ret"
    ...
    if "return_value" in method.keys() or "return_type" in method.keys():
        ...
        if ret_val.type in classes:
            if ret_val.type in builtin_classes:
                result += f"{
                    INDENT * 2}{
                    ret_val.name}.set_gdowner({
                    ret_val.name}.{
                    ret_val.type}_internal_class_ptr.get().get_godot_owner())"
            else:
                result += f"{
                    INDENT * 2}{
                    ret_val.name}.set_gdowner({
...
def generate_string_arg(arg):
    ...
    res += f"{
        INDENT *
        2}cdef String py__string_{
        pythonize_name(
            arg['name'])} = py_c_string_to_string({
                pythonize_name(
                    arg['name'])}.encode('utf-8'))"
    res = generate_newline(res)
    res += f"{INDENT *
...
def generate_method_body_standard(class_, method):
    ...
    if "return_value" in method.keys() or "return_type" in method.keys():
        ...
        if not is_static(method):
            ...
            result += f"{
                INDENT * 2}{
                generate_ret_call(method)} = self.{
                class_['name']}_internal_class_ptr.get().py_{
                pythonize_name(
                    method['name'])}(({
        ...
        if is_property_getter(class_, method["name"]):
            ...
            if ret_val.type == "Variant":
                result += f"{INDENT *
                             2}self.py__{property_name} = <object>_ret"
            else:
        ...
        if untypearray_or_dictionary(
            ...
            res += f"{
                pythonize_name(
                    arg['name'])}.{
                untypearray_or_dictionary(
                    arg['type'])}_internal_class_ptr if {
                pythonize_name(
                    arg['name'])} != None else {
                pythonize_name(
                    arg['name'])}.{
                untypearray_or_dictionary(
                    arg['type'])}_internal_class_ptr, "  # Todo: implement conditional
        elif untypearray_or_dictionary(arg["type"]) in builtin_classes - IGNORED_CLASSES:
            if arg["type"] == "String":
                if is_property_setter(class_, method["name"]):
                    res += f"py__string_{
                        pythonize_name(
                            arg['name'])}.String_internal_class_ptr, "
                else:
                    res += f"py_c_string_to_string({
                        pythonize_name(
                            arg['name'])}.encode('utf-8')).{
                        untypearray_or_dictionary(
                            arg['type'])}_internal_class_ptr, "
            elif arg["type"] == "StringName":
                res += f"py_stringname_{
                    pythonize_name(
                        arg['name'])}.StringName_internal_class_ptr, "
            elif arg["type"] == "NodePath":
                res += f"py_nodepath_{
...
def generate_init(class_):
    ...
    res += f"{
        INDENT *
        2}self.{
        class_['name']}_internal_class_ptr = (CPP{
            class_['name']}.py_new0())"
    res = generate_newline(res)
    return res
...
def generate_cinit(class_):
    ...
    res += f"{
        INDENT *
        2}self.{
        class_['name']}_internal_class_ptr = make_shared[CPP{
            class_['name']}]()"
    res = generate_newline(res)
    ...
    if "inherits" in class_.keys():
        ...
        while cls:
            res += f"{
                INDENT *
                2}self.{
                cls['name']}_internal_class_ptr = make_shared[CPP{
                cls['name']}]()"
            res = generate_newline(res)
            if "inherits" not in cls.keys():
                break
    ...
    if "signals" in cls.keys():
        for signal in cls["signals"]:
            res += f"{
                INDENT *
                2}cdef StringName {
                signal['name']}_name = py_c_string_to_string_name(\"{
                signal['name']}\")"
            res = generate_newline(res)
            res += f"{
                INDENT *
                2}self.{
                signal['name']} = BuiltinSignal(self, {
                signal['name']}_name)"
            res = generate_newline(res)
    return res
...
def generate_get_py_script_method():
    ...
    result = generate_newline(result)
    result += f"{INDENT *
                 2}cdef object script = <object>c_utils.py_get_py_script(id)"
    result = generate_newline(result)
    ...
    result = generate_newline(result)
    result += f"{INDENT *
                 2}_ret.set_gdowner(_ret.String_internal_class_ptr.get().get_godot_owner())"
    result = generate_newline(result)
    result += f"{INDENT * 2}return gd_string_to_py_string(_ret)"
...
def generate_new_static(class_):
    ...
    res += f"{
        INDENT *
        2}self.{
        class_['name']}_internal_class_ptr.get().set_gdowner_{
            class_['name']}(owner)"
    res = generate_newline(res)
    if "inherits" in class_.keys():
        cls = find_class(class_["inherits"])
        while cls:
            res += f"{
                INDENT *
                2}self.{
                cls['name']}_internal_class_ptr.get().set_gdowner_{
                cls['name']}(owner)"
            res = generate_newline(res)
            if "inherits" not in cls.keys():
                break
...
def generate_member_getter(class_, member):
    ...
    if member.type_ != "int" and member.type_ != "float" and member.type_ != "double":
        ...
        body += f"{
            INDENT *
            2}_ret.{
            member.type_}_internal_class_ptr = self.{class_}_internal_class_ptr.get().py_member_get_{
            member.name}()"
        body = generate_newline(body)
        body += f"{
            INDENT *
            2}_ret.set_gdowner(_ret.{
            member.type_}_internal_class_ptr.get().get_godot_owner())"
        body = generate_newline(body)
    else:
        body += f"{
            INDENT *
            2}cdef {
            ungodottype(
                member.type_)} _ret = self.{class_}_internal_class_ptr.get().py_member_get_{
                member.name}()"
    body = generate_newline(body)
...
def generate_member_setter(class_, member):
    ...
    if member.type_ != "int" and member.type_ != "float" and member.type_ != "double":
        body += f"{
            INDENT *
            2}self.{class_}_internal_class_ptr.get().py_member_set_{
            member.name}(value.{
            member.type_}_internal_class_ptr)"
    else:
        body += f"{
            INDENT *
            2}self.{class_}_internal_class_ptr.get().py_member_set_{
...
def generate_property(property, classname):
    ...
    result = generate_newline(result)
    result += f"{INDENT *
                 2}cdef _ret = self. {pythonize_name(property['getter'])}({generate_property_index(property)})"
    result = generate_newline(result)
    ...
    if "setter" in property and property["setter"] != "":
        ...
        if (classname ==
            ...
            result += f"{INDENT}def {
                pythonize_name(
                    property['name'])}(self, Array value):"
        ...
        result = generate_newline(result)
        result += f"{INDENT *
                     2}self.{pythonize_name(property['setter'])}({generate_property_index(property, True)}value)"
        result = generate_newline(result)
            ...
            for method in class_["methods"]:
                if ("return_value" in method.keys()):
                    if (unbitfield_type(get_class_from_enum(
                        ...
                        if "enum" not in method["return_value"]["type"]:
...
def generate_constructor(classname):
    ...
    res = generate_newline(res)
    res += f"{INDENT *
              2}cdef {classname} class_ = {classname}.__new__({classname})"
    res = generate_newline(res)
    res += f"{
        INDENT *
        2}class_.{
        class_['name']}_internal_class_ptr = construct_{
            class_['name']}()"
    res = generate_newline(res)
    res += f"{
        INDENT *
        2}class_.set_gdowner(class_.{
        class_['name']}_internal_class_ptr.get().get_godot_owner())"
    res = generate_newline(res)
            ...
            if operator in operator_to_method.keys():
                ...
                for target in op.right_type_values:
                    ...
                    if target in {"float", "int", "bool", "Nil"}:
                        res += f"{INDENT *
                                  2}cdef {ungodottype(target)} primitive_val_{target}"
                        res = generate_newline(res)
                    elif target in builtin_classes.union(classes):
                        res += f"{INDENT *
                                  2}cdef {target} complex_val_{target}"
                        res = generate_newline(res)
                    elif target == "Variant":
                        res += f"{INDENT *
                                  2}cdef PyVariant complex_val_variant"
                        res = generate_newline(res)
                    if class_name not in typed_arrays_names:
                        res += f"{INDENT *
                                  2}if isinstance(other, {unvariant(get_instance_type(target))}):"
                    else:
                        res += f"{
                            INDENT *
                            2}if isinstance(other, {
                            unvariant_type_array(
                    ...
                    if target in {"float", "int", "bool", "Nil"}:
                        res += f"{INDENT *
                                  3}primitive_val_{target} = <{ungodottype(target)}>other"
                        res = generate_newline(res)
                    elif target in builtin_classes.union(classes):
                        res += f"{INDENT *
                                  3}complex_val_{target} = <{target}>other"
                        res = generate_newline(res)
                    elif target == "Variant":
                        res += f"{INDENT *
                                  3}complex_val_variant = create_variant_from_py_object(other)"
                        res = generate_newline(res)
                    ...
                    if op.return_type in builtin_classes - \
                            {"float", "int", "bool"}:
                        res += f"{INDENT * 3}_ret.{op.return_type}_internal_class_ptr = {address_param('self',
                                                                                                       class_name,
                                                                                                       True)}.py_operator_{operator_to_python_name(operator)}({address_param('other',
                                                                                                                                                                             target,
                                                                                                                                                                             False)})"
                    # I don't know the correct type, use only one
                    elif op.return_type in {"Variant", "PyVariant", "object"}:
                        res += f"{INDENT * 3}_ret = {address_param('self',
                                                                   class_name,
                                                                   True)}.py_operator_{operator_to_python_name(operator)}({address_param('other',
                                                                                                                                         target,
                                                                                                                                         False)}).get_converted_value()"
                    else:
                        res += f"{INDENT * 3}_ret = {address_param('self',
                                                                   class_name,
                                                                   True)}.py_operator_{operator_to_python_name(operator)}({address_param('other',
                ...
                res += f"{INDENT *
                          3}raise Exception(f'type \"'+type(other)+'\" not supported')"
                res = generate_newline(res)
...
def should_skip_import(classname, class_to_import):
    return classname == "Node" and class_to_import in {
...
def generate_classes(classes, filename, is_core=False, is_typed_array=False):
    ...
    if is_typed_array:
        ...
        res = generate_newline(res)
        res += f"from py4godot.classes.cpp_bridge cimport construct_{
            classes[0]['name']}"
        res = generate_newline(res)
        res += f"from py4godot.classes.cpp_bridge cimport cast_to_{
            classes[0]['name']}"
        res = generate_newline(res)
        classes_to_import = get_classes_to_import(classes)
        for cls in classes_to_import:
...
def generate_dictionary_set_item():
    ...
    res = generate_newline(res)
    res += f"{INDENT *
              2}cdef Variant var = self.Dictionary_internal_class_ptr.get()[0][create_variant_from_py_object(key).variant]"
    res = generate_newline(res)
    res += f"{INDENT * 2}cdef str typename = str(type(value).__name__)"
    res = generate_newline(res)
    res += f"{INDENT *
              2}var.init_from_py_object(<PyObject*>value, typename.encode('utf-8'))"
    return res
...
def generate_dictionary_get_item():
    ...
    res = generate_newline(res)
    res += f"{INDENT *
              2}cdef PyObject * pyobject = self.Dictionary_internal_class_ptr.get()[0][create_variant_from_py_object(key).variant].get_converted_value(True)"
    res = generate_newline(res)
...
def generate_get_item_from_type_array(classname, classtype):
    ...
    if classtype in builtin_classes - {"int", "float", "bool", "Nil"}:
        ...
        res += f"{INDENT *
                  2}return self.{classname}_internal_class_ptr.get()[0][<int>index]"
    return res
...
def generate_array_set_item(class_):
    ...
    if class_["name"] == "PackedInt32Array":
        ...
        res += f"{INDENT *
                  2}(&self.{class_['name']}_internal_class_ptr.get()[0][index])[0] = <int>value"
        ...
        res += f"{INDENT *
                  2}(&self.{class_['name']}_internal_class_ptr.get()[0][index])[0] = value"
        ...
        res += f"{
            INDENT *
            2}(&self.{
            class_['name']}_internal_class_ptr.get()[0][index])[0] = (<Color>value).Color_internal_class_ptr"
        ...
        res += f"{
            INDENT *
            2}(&self.{
            class_['name']}_internal_class_ptr.get()[0][index])[0] = (<Vector3>value).Vector3_internal_class_ptr"
        ...
        res += f"{
            INDENT *
            2}(&self.{
            class_['name']}_internal_class_ptr.get()[0][index])[0] = (<Vector2>value).Vector2_internal_class_ptr"
        ...
        res += f"{
            INDENT *
            2}(&self.{
            class_['name']}_internal_class_ptr.get()[0][index])[0] = (<String>value).String_internal_class_ptr"
        ...
        res += f"{
            INDENT *
            2}self.{
            class_['name']}_internal_class_ptr.get()[0][index].init_from_py_object(<PyObject*>value, type(value).__name__.encode('utf-8'))"
...
def generate_del(class_):
    if class_["name"] not in builtin_classes and class_[
        ...
        if is_refcounted(class_):
            res += f"{INDENT *
                      2}if not is_ptr_null(self.RefCounted_internal_class_ptr) and self.casted_from is None:"
            res = generate_newline(res)
            res += f"{INDENT *
                      3}self.RefCounted_internal_class_ptr.get().py_destroy_ref()"
            res = generate_newline(res)
            return res
        else:
            res += f"{INDENT * 2}pass"
        ...
        res += f"{
            INDENT *
            2}if not is_ptr_null(self.{
            class_['name']}_internal_class_ptr) and self.shouldBeDeleted:"
        res = generate_newline(res)
        res += f"{
            INDENT *
            3}self.{
            class_['name']}_internal_class_ptr.get().{
            class_['name']}_py_destroy()"
        res = generate_newline(res)
        ...
        res = generate_newline(res)
        res += f"{INDENT *
                  2}if is_ptr_null(self.{class_['name']}_internal_class_ptr):"
        res = generate_newline(res)
        res += f"{INDENT * 3}pass"
        res = generate_newline(res)
    return res
...
def generate_cast(class_):
    ...
    res += f"{
        INDENT *
        2}cls.{
        class_['name']}_internal_class_ptr = cast_to_{
            class_['name']}(other.Object_internal_class_ptr.get())"
    res = generate_newline(res)
    res += (f"{INDENT * 2}cls"
...
def generate_special_methods_packed_array(class_):
    ...
    res += f"{
        INDENT *
        2}cdef vector[{type_}] value_vector = self.{
        class_['name']}_internal_class_ptr.get()[0].to_vector()"
    res = generate_newline(res)
    res += f"{INDENT * 2}cdef Py_ssize_t size = value_vector.size()"
    res = generate_newline(res)
    res += f"{INDENT *
              2}cdef {type_}[:] memory_view = <{type_}[:size]>value_vector.data()"
    res = generate_newline(res)
    ...
    res = generate_newline(res)
    res += f"{INDENT *
              2}cdef {type_}* value_ptr = self.{class_['name']}_internal_class_ptr.get()[0].get_pointer()"
    res = generate_newline(res)
    res += f"{INDENT * 2}cdef Py_ssize_t size = self.size()"
    res = generate_newline(res)
    res += f"{INDENT *
              2}cdef {type_}[:] memory_view = <{type_}[:size]>value_ptr"
    res = generate_newline(res)
    ...
    res += f"{
        ...
        2}array.{
        class_['name']}_internal_class_ptr = (CPP{
            class_['name']}.py_from_ptr(value_ptr, size))"
    res = generate_newline(res)
...
def generate_from_list_array(class_):
    ...
    res = generate_newline(res)
    res += f"{INDENT *
              2}cdef {class_['name']} result = {class_['name']}.new0()"
    res = generate_newline(res)
...
def generate_variant_checks(method):
    ...
    for arg in args:
        if arg["type"] == "Variant":
            res += f"{
                INDENT *
                2}if not (type({
                arg['name']}).__name__ in variant_types.core_type or isinstance({
                arg['name']}, py4godot_object.Object)):"
            res = generate_newline(res)
    ...
    if method["is_vararg"]:
        ...
        res = generate_newline(res)
        res += f"{INDENT *
                  3}if not (type(arg).__name__ in variant_types.core_type or isinstance(arg, py4godot_object.Object))"
        res = generate_newline(res)
...
if __name__ == "__main__":
    ...
    with open('py4godot/gdextension-api/extension_api.json', 'r') as myfile:
        ...
        for class_ in obj["classes"]:
            generate_classes([class_],
                             f"py4godot/classes/{class_['name']}.pyx")
        ...
        for array in arrays:
            generate_classes([array],
                             f"py4godot/classes/{array['name']}.pyx",
                             is_core=False,
                             is_typed_array=True)

        generate_classes(
            obj["builtin_classes"],
            f"py4godot/classes/core.pyx",
            is_core=True)
