...
def compile_python_ver_file(platform):
    ...
    with open("platforms/binary_dirs/python_ver_temp.cross", "r") as python_temp:
        ...
        with open(
            "platforms/binary_dirs/python_ver_compile.cross", "w"
        ) as python_compile:
...
def get_compiler():
    compiler_res = subprocess.run(
        "vcvarsall", shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT
    )
    if compiler_res.returncode == 0:
        return "msvc"

    compiler_res = subprocess.run(
        "gcc --version", shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT
    )
    if compiler_res.returncode == 0:
        return "gcc"
...
my_parser.add_argument(
    "--compiler", help="specify the compiler, you want to use to compile"
)
my_parser.add_argument(
    "--target_platform", help="specify the platform, you want to go build for"
)
...
my_parser.add_argument(
    "-create_plugin", help="Should this create a plugin", default="True"
)
my_parser.add_argument(
    "-buildtype",
...
msvc_init = (
    f"vcvarsall.bat {'x86_amd64'} {command_separator} cl {command_separator} "
...
try:
    ...
    if should_create_plugin.lower() == "true":
        ...
        copytree(
            f"build/final/{args.target_platform}/{python_ver}-{args.target_platform}",
            f"build/py4godot/{python_ver}-{args.target_platform}",
        )
        shutil.copy(
            "build_resources/dependencies.txt", "build/py4godot/dependencies.txt"
        )
        shutil.copy(
        ...
        shutil.copy(
            "build_resources/python.gdextension", "build/py4godot/python.gdextension"
        )
        shutil.copy(
            "build_resources/export_py4godot.gd", "build/py4godot/export_py4godot.gd"
        )
        shutil.copy(
            "build_resources/export_py4godot_main.gd",
            "build/py4godot/export_py4godot_main.gd",
        )
        shutil.copy("build_resources/plugin.cfg", "build/py4godot/plugin.cfg")
        shutil.copy(
            "build_resources/signal_script.py", "build/py4godot/signal_script.py"
        )
