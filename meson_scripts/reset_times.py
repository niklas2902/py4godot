...
# As each time, we install cython, when using the cached version, it gets
# a new timestamp, we reset the timestamp
import glob
import os
import time

files = glob.glob(
    "C:/hostedtoolcache/windows/Python/3.11.3/x64/Lib/site-packages/Cython/Includes/libcpp/*.pxd")
...
# Desired modification time (in the format: Year, Month, Day, Hour,
# Minute, Second)
desired_time = (2023, 1, 15, 12, 30, 0)

# Convert the desired time to a Unix timestamp
# Adding dummy values for timezone and DST correction
timestamp = time.mktime(desired_time + (0, 0, -1))
