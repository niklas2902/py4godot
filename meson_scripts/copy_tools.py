...
def strip_platform(text):
    text = text[1:]
    return text.lstrip("linuxarm64").lstrip("linux64").lstrip(
        "windows64").lstrip("windows32").lstrip("linux32").lstrip("darwin64")
...
def run(platform):
    ...
    if "windows" in platform:
        list_dll = list(
            filter(
                lambda pathname: not (
                    pathname.endswith("main.dll") or pathname.endswith("pythonscript.dll")),
                glob.glob(
                    "**/*.dll",
                    recursive=True)))
        ...
        list_dll = list(
            filter(
                lambda pathname: not (
                    pathname.endswith("main.dylib") or pathname.endswith("pythonscript.dylib")),
                glob.glob(
                    "**/*.dylib",
                    recursive=True)))
                ...
                copy(
                    ...
                    f"build/final/{platform}/{python_ver}-{platform}/python/Lib/site-packages/" + strip_platform(
                        entry.lstrip("build").replace(
                            ...
                            "/")). replace(
                        ".dll",
                        ".pyd"))  # dst can be a folder; use copy2() to preserve timestamp
            else:
                os.makedirs(
                    os.path.dirname(
...
def copy_main(platform):
    if "windows" in platform:
        # This is a weird phenomenon of godot 4.2. It copies the dll and
        # renames it. Thus, we have to change what we depend on
