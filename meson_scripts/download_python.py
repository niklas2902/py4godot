...
def download_file(platform, allow_copy=False):
    ...
    if platform != "linux32" and platform != "linux64":
        url = f'https://github.com/astral-sh/python-build-standalone/releases/download/20260211/{python_ver}+20260211-{
            platform_dict[platform]}.tar.gz'
        python_file = f'{python_files_dir}/{python_ver}-{
            platform_dict[platform]}.tar.gz'
    ...
    if (not os.path.isfile(python_file)
        ):  # checking whether file was already downloaded
...
def extract_archive(file, export_name):
    ...
    if (not os.path.isdir(python_files_dir + "/" + export_name)):
        ...
        if file.endswith("tar") or file.endswith("gz"):
            ...
            my_tar.extractall(python_files_dir + "/" + export_name)
            my_tar.close()
        else:
...
def copy_to_build(export_folder, platform):
    ...
    if "linux" in platform:
        ...
        if os.path.exists(python_bin):
            ...
            os.chmod(python_bin, st.st_mode | stat.S_IXUSR |
...
def delete_pip(platform, export_folder):
    ...
    if "windows" in platform:
        print("deleting pip...")
        if os.path.isdir(
                f"python_files/{export_folder}/python/Lib/site-packages/pip"):
            shutil.rmtree(
                f"python_files/{export_folder}/python/Lib/site-packages/pip")
