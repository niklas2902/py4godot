...
class TestObject(Node3D):
    ...
    def _ready(self) -> None:
        ...
        self.custom_signal_with_args.connect(
            self.custom_signal_with_arg_function)
    ...
    def disconnect_custom_signal_with_arg(self) -> None:
        self.custom_signal_with_args.disconnect(
            self.custom_signal_with_arg_function)

    def disconnect_custom_signal(self) -> None:
        self.custom_signal_no_args.disconnect(
            self.custom_signal_no_arg_function)
    ...
    def check_wait_emit(self):
        future = asyncio.run_coroutine_threadsafe(
            self.wait_emit_execute(), get_event_loop())
