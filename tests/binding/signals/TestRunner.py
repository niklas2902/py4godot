...
class TestRunner(Node3D):
    ...
    def _process(self, delta: float) -> None:
        print("##########start#############")
        test_object = self.get_node(
            self.test_object_path
        ).get_pyscript()  # Replace with the actual object you're testing
        ...
        suite.addTest(
            PythonTest("test_custom_signal_with_args", test_object=test_object)
        )
        suite.addTest(
            PythonTest("test_custom_signal_godot_object", test_object=test_object)
        )
        suite.addTest(PythonTest("test_wait_emit", test_object=test_object))
        suite.addTest(PythonTest("test_lambdas", test_object=test_object))
