...
class PythonTest(unittest.TestCase):
    ...
    def test_distance_to(self):
        ...
        self.assertEqual(
            Vector3(1, 1, 1).distance_to(Vector3(0, 0, 0)), 1.7320507764816284
        )
    ...
    def test_angle_to(self):
        self.assertEqual(
            Vector3(1, 0, 0).angle_to(Vector3(0, 1, 0)), 1.5707963705062866
        )
        self.assertEqual(Vector3(1, 0, 0).angle_to(Vector3(0.5, 0, 0)), 0)
        self.assertEqual(
            Vector3(1, 0, 0).angle_to(Vector3(0.5, 1, 0)), 1.1071487665176392
        )
        self.assertEqual(
            Vector3(1, 0, 0).angle_to(Vector3(0, 1, 1)), 1.5707963705062866
        )
        self.assertEqual(
            Vector3(100, 0, 0).angle_to(Vector3(0, 1, 0)), 1.5707963705062866
        )

    def test_linear_interpolate(self):
        self.assertEqual(
            Vector3(1, 1, 1).linear_interpolate(Vector3(1, 2, 3), 1), Vector3(1, 2, 3)
        )
        self.assertEqual(
            Vector3(1, 1, 1).linear_interpolate(Vector3(1, 2, 3), 2), Vector3(1, 3, 5)
        )
        self.assertEqual(
            Vector3(1, 1, 1).linear_interpolate(Vector3(1, 2, 3), 0), Vector3(1, 1, 1)
        )

    def test_cubic_interpolate(self):
        # TODO: Find out what this does
        self.assertEqual(
            Vector3(1, 1, 1).cubic_interpolate(
                Vector3(1, 2, 3), Vector3(1, 2, 3), Vector3(1, 2, 3), 5
            ),
            Vector3(1, -124, -249),
        )
    ...
    def test_rotated(self):
        self.assertEqual(
            Vector3(1, 0, 0)
            .rotated(Vector3(0, 1, 0), 3.14159265359)
            .get_axis(Vector3_Axis.X.value),
            -1,
        )
        self.assertEqual(
            Vector3(1, 0, 0)
            .rotated(Vector3(0, 1, 0), 0)
            .get_axis(Vector3_Axis.X.value),
            1,
        )
        self.assertEqual(
            int(
                Vector3(1, 0, 0)
                .rotated(Vector3(0, 1, 0), 1.57079632679)
                .get_axis(Vector3_Axis.X.value)
            ),
            0,
        )
    ...
    def test_snapped(self):
        ...
        self.assertEqual(
            Vector3(1.3, 3.21, 0).snapped(Vector3(1, 0.1, 1)), Vector3(1, 3.2, 0)
        )
        self.assertEqual(
            Vector3(1.3, 3.21, 2.21).snapped(Vector3(1, 0.1, 0.1)), Vector3(1, 3.2, 2.2)
        )
    ...
    def test_divide_vector(self):
        self.assertEqual(
            Vector3(1, 2, 3).divide_vector(Vector3(1, 2, 3)), Vector3(1, 1, 1)
        )
        self.assertEqual(
            Vector3(1, 2, 3).divide_vector(Vector3(-1, -2, -3)), Vector3(-1, -1, -1)
        )
