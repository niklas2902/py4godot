CORE_VALS={"godot_string":"String", "godot_aabb":"AABB", "godot_basis":"Basis", "godot_color":"Color",
           "godot_dictionary":"Dictionary", "godot_node_path":"NodePath", "godot_plane":"Plane",
           "godot_pool_array":"PoolArray", "godot_quat":"Quat", "godot_rect2":"Rect2", "godot_rid":"RID",
           "godot_string_name":"StringName", "godot_transform":"Transform", "godot_variant":"Variant",
           "godot_vector2":"Vector2", "godot_vector3":"Vector3", "godot_array":"Array"}
EXCLUDE_WORDS = ["import", "raise", "class", "from", "pass", "with", "global", "print", "short", "in", "args"]

import re
input = """void (*godot_string_new)(godot_string *r_dest);
...
def generate_args(args):
    ...
    for arg in args:
        ...
        res = re.match("(?P<type>.*) (?P<name>.*)",arg)

        type = transform_val(res.group("type").strip())
        name = res.group("name")
        name = transform_name(name.replace('*',"").replace('p_',"").replace('r_',""))
        if(name.strip().startswith("self")):
            string += "self,"
            continue
        string += f"{type} {name.strip()},"
    return string.strip(",")

def transform_val(type_):
    if (type_ in CORE_VALS):
        type_ = CORE_VALS[type_]
    return type_
...
def generate_method_body(method_name, args, return_):
    res = ""
    return_type = return_.strip()
    if(return_.strip() in CORE_VALS):
        return_type = transform_val(return_.strip())

    ret_string = ""
    args_string = f"("
    for arg in args:
        if("self" in arg):
            args_string += "&self._native,"
            continue
        arg = arg.replace("const ",'')
        res = re.match("(?P<type>.*?) (?P<name>.*)", arg.strip())

        type = transform_val(res.group("type"))
        name = res.group("name")
        name = transform_name(name.replace('*',"").replace('p_',"").replace("r_",""))
        if(type in CORE_VALS.values()):
            args_string += f"&{name}._native,"
        else:
            args_string += name+","
    args_string = args_string.rstrip(",")+")"
    res =f"api_core.godot_string_{method_name}{args_string}"
    if("void" not in return_):
        if(return_.strip() in CORE_VALS):
            res = f"return {return_type}.new_static({res})"
        else:
            res = f"return {res}"
    return res
...
for line in input.split("\n"):
    ...
    line = line.strip()
    res = re.match("(?P<return>.*?)\(\*godot_string_(?P<name>.*?)\)\((?P<args>.*?)\)", line)
    if(res):
        method = f"""def {res.group('name')}({(generate_args(res.group('args').split(','))).strip(", ")}):
    {generate_method_body(res.group("name"),res.group('args').split(","), res.group("return"))}"""
        print(method)
    else:
        print("res not found")
    print()