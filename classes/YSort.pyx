...
############################Generated class###################################
cdef class YSort(classes.Node2D.Node2D):
  def __init__(self):
    super().__init__()
...
##################################Generated Methods#########################################
  ...
  def  set_sort_enabled(self,  bool enabled):
    cdef godot_object *_owner = self.godot_owner

    cdef void *args[1]

    args[0] = &enabled
    api_core.godot_method_bind_ptrcall(bind_ysort_set_sort_enabled,self.godot_owner,args,NULL)
    hello('hallo2')
