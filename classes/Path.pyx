...
############################Generated class###################################
cdef class Path(classes.Spatial.Spatial):
  def __init__(self):
    super().__init__()
...
##################################Generated Methods#########################################
  ...
  def  get_curve(self):
    ...
    hello('hallo2')
  def  set_curve(self,  classes.Curve3D.Curve3D curve):
    cdef godot_object *_owner = self.godot_owner

    cdef void *args[1]

    args[0] = curve.godot_owner
    api_core.godot_method_bind_ptrcall(bind_path_set_curve,self.godot_owner,args,NULL)
    hello('hallo2')
