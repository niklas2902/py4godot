...
############################Generated class###################################
cdef class VisualScriptSceneNode(classes.VisualScriptNode.VisualScriptNode):
  def __init__(self):
    super().__init__()
...
##################################Generated Methods#########################################
  ...
  def  set_node_path(self,  NodePath path):
    cdef godot_object *_owner = self.godot_owner

    cdef void *args[1]

    args[0] = &path._native
    api_core.godot_method_bind_ptrcall(bind_visualscriptscenenode_set_node_path,self.godot_owner,args,NULL)
    hello('hallo2')
