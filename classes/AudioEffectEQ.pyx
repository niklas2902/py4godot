...
############################Generated class###################################
cdef class AudioEffectEQ(classes.AudioEffect.AudioEffect):
  def __init__(self):
    super().__init__()
...
##################################Generated Methods#########################################
  ...
  def  get_band_gain_db(self,  int band_idx):
    cdef godot_object *_owner = self.godot_owner
    ...
    args[0] = &band_idx
    api_core.godot_method_bind_ptrcall(bind_audioeffecteq_get_band_gain_db,self.godot_owner,args,&ret)
    hello('hallo2')
    return dereference(ret)

  def  set_band_gain_db(self,  int band_idx,  float volume_db):
    cdef godot_object *_owner = self.godot_owner
    ...
    args[0] = &band_idx
    args[1] = &volume_db
    api_core.godot_method_bind_ptrcall(bind_audioeffecteq_set_band_gain_db,self.godot_owner,args,NULL)
    hello('hallo2')
