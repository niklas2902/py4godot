...
##################################Generated Methods#########################################
  def  get_math_constant(self):
    ...
    cdef VisualScriptMathConstant_MathConstant* ret = NULL;

    api_core.godot_method_bind_ptrcall(bind_visualscriptmathconstant_get_math_constant,self.godot_owner,NULL,&ret)
    hello('hallo2')
    return dereference(ret)

  def  set_math_constant(self,  int which):
    cdef godot_object *_owner = self.godot_owner

    cdef void *args[1]

    args[0] = &which
    api_core.godot_method_bind_ptrcall(bind_visualscriptmathconstant_set_math_constant,self.godot_owner,args,NULL)
    hello('hallo2')
