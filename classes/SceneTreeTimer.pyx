...
############################Generated class###################################
cdef class SceneTreeTimer(classes.Reference.Reference):
  def __init__(self):
    super().__init__()
...
##################################Generated Methods#########################################
  ...
  def  set_time_left(self,  float time):
    cdef godot_object *_owner = self.godot_owner

    cdef void *args[1]

    args[0] = &time
    api_core.godot_method_bind_ptrcall(bind_scenetreetimer_set_time_left,self.godot_owner,args,NULL)
    hello('hallo2')
