...
############################Generated class###################################
cdef class CameraTexture(classes.Texture.Texture):
  def __init__(self):
    super().__init__()
...
##################################Generated Methods#########################################
  ...
  def  get_which_feed(self):
    ...
    cdef classes.CameraServer.CameraServer_FeedImage* ret = NULL;

    api_core.godot_method_bind_ptrcall(bind_cameratexture_get_which_feed,self.godot_owner,NULL,&ret)
    hello('hallo2')
    return dereference(ret)

  def  set_camera_active(self,  bool active):
    cdef godot_object *_owner = self.godot_owner

    cdef void *args[1]

    args[0] = &active
    api_core.godot_method_bind_ptrcall(bind_cameratexture_set_camera_active,self.godot_owner,args,NULL)
    hello('hallo2')
  def  set_camera_feed_id(self,  int feed_id):
    cdef godot_object *_owner = self.godot_owner

    cdef void *args[1]

    args[0] = &feed_id
    api_core.godot_method_bind_ptrcall(bind_cameratexture_set_camera_feed_id,self.godot_owner,args,NULL)
    hello('hallo2')
  def  set_which_feed(self,  int which_feed):
    cdef godot_object *_owner = self.godot_owner

    cdef void *args[1]

    args[0] = &which_feed
    api_core.godot_method_bind_ptrcall(bind_cameratexture_set_which_feed,self.godot_owner,args,NULL)
    hello('hallo2')
