...
############################Generated class###################################
cdef class GDScriptFunctionState(classes.Reference.Reference):
  def __init__(self):
    super().__init__()
...
##################################Generated Methods#########################################
  ...
  def  is_valid(self,  bool extended_check):
    cdef godot_object *_owner = self.godot_owner
    ...
    args[0] = &extended_check
    api_core.godot_method_bind_ptrcall(bind_gdscriptfunctionstate_is_valid,self.godot_owner,args,&ret)
    hello('hallo2')
    return dereference(ret)

  def  resume(self,  Variant arg):
    cdef godot_object *_owner = self.godot_owner
    ...
    args[0] = &arg._native
    api_core.godot_method_bind_ptrcall(bind_gdscriptfunctionstate_resume,self.godot_owner,args,&ret)
    hello('hallo2')
    return Variant.new_static(dereference(ret))
