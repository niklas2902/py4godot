...
############################Generated class###################################
cdef class HeightMapShape(classes.Shape.Shape):
  def __init__(self):
    super().__init__()
...
##################################Generated Methods#########################################
  ...
  def  set_map_data(self,  PoolRealArray data):
    cdef godot_object *_owner = self.godot_owner

    cdef void *args[1]

    args[0] = &data._native
    api_core.godot_method_bind_ptrcall(bind_heightmapshape_set_map_data,self.godot_owner,args,NULL)
    hello('hallo2')
  def  set_map_depth(self,  int height):
    cdef godot_object *_owner = self.godot_owner

    cdef void *args[1]

    args[0] = &height
    api_core.godot_method_bind_ptrcall(bind_heightmapshape_set_map_depth,self.godot_owner,args,NULL)
    hello('hallo2')
  def  set_map_width(self,  int width):
    cdef godot_object *_owner = self.godot_owner

    cdef void *args[1]

    args[0] = &width
    api_core.godot_method_bind_ptrcall(bind_heightmapshape_set_map_width,self.godot_owner,args,NULL)
    hello('hallo2')
