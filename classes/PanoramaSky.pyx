...
############################Generated class###################################
cdef class PanoramaSky(classes.Sky.Sky):
  def __init__(self):
    super().__init__()
...
##################################Generated Methods#########################################
  def  get_panorama(self):
    ...
    hello('hallo2')
  def  set_panorama(self,  classes.Texture.Texture texture):
    cdef godot_object *_owner = self.godot_owner

    cdef void *args[1]

    args[0] = texture.godot_owner
    api_core.godot_method_bind_ptrcall(bind_panoramasky_set_panorama,self.godot_owner,args,NULL)
    hello('hallo2')
