...
############################Generated class###################################
cdef class ARVROrigin(classes.Spatial.Spatial):
  def __init__(self):
    super().__init__()
...
##################################Generated Methods#########################################
  ...
  def  set_world_scale(self,  float world_scale):
    cdef godot_object *_owner = self.godot_owner

    cdef void *args[1]

    args[0] = &world_scale
    api_core.godot_method_bind_ptrcall(bind_arvrorigin_set_world_scale,self.godot_owner,args,NULL)
    hello('hallo2')
