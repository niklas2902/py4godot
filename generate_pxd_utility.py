...
def generate_method(mMethod):
    ...
    def_function = f"{INDENT}{unvarianttype(ungodottype(untypearray(get_ret_value(mMethod))))} py_{pythonize_name(mMethod['name'])}({args});"
    res = generate_newline(res)
    res += def_function
    res = generate_newline(res)
    return res
...
if __name__ == "__main__":
    with open('py4godot/gdextension-api/extension_api.json', 'r') as myfile:
        ...
        with open("py4godot/classes/py_utils.pxd", "w") as f:
            f.write(res)